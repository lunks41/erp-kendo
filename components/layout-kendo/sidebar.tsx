...
export function Sidebar({ companyId }: SidebarProps) {
  ...
  return (
    <aside className="flex w-48 flex-col border-r border-slate-200 bg-slate-50 dark:border-slate-700 dark:bg-slate-900/50">
      <nav className="flex-1 space-y-0.5 p-3">
        <div className="mb-3">
          <p className="px-2 text-[10px] font-semibold uppercase tracking-wider text-slate-500 dark:text-slate-400">
            Master
          </p>
        </div>
        {navItems.map((item) => {
          ...
          return (
            ...
            >
              ...
              <ChevronRight
                className={`h-3.5 w-3.5 shrink-0 ${isActive ? "opacity-100" : "opacity-50"}`}
              />
            </Link>
          );
        })}
