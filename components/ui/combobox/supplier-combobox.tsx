...
/**
 * Supplier ComboBox with ERP-style filtering:
...
export function SupplierCombobox({
  ...
  const { data = [], isLoading } = useSupplierLookup();
  const [filteredData, setFilteredData] = useState<ISupplierLookup[]>(data);
  ...
  const resolvedValue = useMemo(() => {
    if (value == null || value.supplierId == null) return null;
    const id = Number(value.supplierId);
    if (!id) return null;
    const found = data.find((item) => Number(item.supplierId) === id);
    return found ?? value;
  }, [value, data]);
