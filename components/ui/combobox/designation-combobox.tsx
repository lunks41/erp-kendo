...
import type { IDesignationLookup } from "@/interfaces/lookup";

/**
 * Designation ComboBox with ERP-style filtering:
...
export interface DesignationComboboxProps {
  value?: IDesignationLookup | null;
  onChange?: (value: IDesignationLookup | null) => void;
  onBlur?: () => void;
  isDisable?: boolean;
  placeholder?: string;
  label?: string;
...
export function DesignationCombobox({
  ...
  const { data = [], isLoading } = useDesignationLookup();
  const [filteredData, setFilteredData] = useState<IDesignationLookup[]>(data);
  ...
  const resolvedValue = useMemo(() => {
    if (value == null || value.designationId == null) return null;
    const id = Number(value.designationId);
    if (!id) return null;
    const found = data.find((item) => Number(item.designationId) === id);
    return found ?? value;
  }, [value, data]);
