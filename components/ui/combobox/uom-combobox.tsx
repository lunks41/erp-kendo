...
/**
 * UOM (Unit of Measure) ComboBox with ERP-style filtering:
 ...
 */
export interface UomComboboxProps {
  value?: IUomLookup | null;
  onChange?: (value: IUomLookup | null) => void;
  onBlur?: () => void;
  isDisable?: boolean;
  placeholder?: string;
  label?: string;
...
export function UomCombobox({
  ...
  const { data = [], isLoading } = useUomLookup();
  const [filteredData, setFilteredData] = useState<IUomLookup[]>(data);
  ...
  const resolvedValue = useMemo(() => {
    if (value == null || value.uomId == null) return null;
    const id = Number(value.uomId);
    if (!id) return null;
    const found = data.find((item) => Number(item.uomId) === id);
    return found ?? value;
  }, [value, data]);
