...
import type { IProductLookup } from "@/interfaces/lookup";

/**
 * Product ComboBox with ERP-style filtering:
...
export interface ProductComboboxProps {
  value?: IProductLookup | null;
  onChange?: (value: IProductLookup | null) => void;
  onBlur?: () => void;
  isDisable?: boolean;
  placeholder?: string;
  label?: string;
...
export function ProductCombobox({
  ...
  const { data = [], isLoading } = useProductLookup();
  const [filteredData, setFilteredData] = useState<IProductLookup[]>(data);
  ...
  const resolvedValue = useMemo(() => {
    if (value == null || value.productId == null) return null;
    const id = Number(value.productId);
    if (!id) return null;
    const found = data.find((item) => Number(item.productId) === id);
    return found ?? value;
  }, [value, data]);
