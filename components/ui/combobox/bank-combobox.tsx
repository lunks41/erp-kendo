...
import type { IBankLookup } from "@/interfaces/lookup";

/**
 * Bank ComboBox with ERP-style filtering:
...
export function BankCombobox({
  ...
  const { data = [], isLoading } = useBankLookup();
  const [filteredData, setFilteredData] = useState<IBankLookup[]>(data);
  ...
  const resolvedValue = useMemo(() => {
    if (value == null || value.bankId == null) return null;
    const id = Number(value.bankId);
    if (!id) return null;
    const found = data.find((item) => Number(item.bankId) === id);
    return found ?? value;
  }, [value, data]);
