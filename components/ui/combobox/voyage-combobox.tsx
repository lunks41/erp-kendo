...
import type { IVoyageLookup } from "@/interfaces/lookup";

/**
 * Voyage ComboBox with ERP-style filtering:
 ...
 */
export interface VoyageComboboxProps {
  value?: IVoyageLookup | null;
  onChange?: (value: IVoyageLookup | null) => void;
  onBlur?: () => void;
  isDisable?: boolean;
  placeholder?: string;
  label?: string;
...
export function VoyageCombobox({
  ...
  const { data = [], isLoading } = useVoyageLookup();
  const [filteredData, setFilteredData] = useState<IVoyageLookup[]>(data);
  ...
  const resolvedValue = useMemo(() => {
    if (value == null || value.voyageId == null) return null;
    const id = Number(value.voyageId);
    if (!id) return null;
    const found = data.find((item) => Number(item.voyageId) === id);
    return found ?? value;
  }, [value, data]);
