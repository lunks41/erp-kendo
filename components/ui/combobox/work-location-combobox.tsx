...
/**
 * Work Location ComboBox with ERP-style filtering:
 ...
 */
export interface WorkLocationComboboxProps {
  value?: IWorkLocationLookup | null;
  onChange?: (value: IWorkLocationLookup | null) => void;
  onBlur?: () => void;
  isDisable?: boolean;
  placeholder?: string;
  label?: string;
...
export function WorkLocationCombobox({
  value = null,
  onChange,
  onBlur,
  isDisable = false,
  ...
  className,
  id,
}: WorkLocationComboboxProps) {
  const { data = [], isLoading } = useWorkLocationLookup();
  const [filteredData, setFilteredData] = useState<IWorkLocationLookup[]>(data);
  ...
  const resolvedValue = useMemo(() => {
    if (value == null || value.workLocationId == null) return null;
    const id = Number(value.workLocationId);
    if (!id) return null;
    const found = data.find((item) => Number(item.workLocationId) === id);
    return found ?? value;
  }, [value, data]);
