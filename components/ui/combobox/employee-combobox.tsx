...
import type { IEmployeeLookup } from "@/interfaces/lookup";

/**
 * Employee ComboBox with ERP-style filtering:
...
export interface EmployeeComboboxProps {
  value?: IEmployeeLookup | null;
  onChange?: (value: IEmployeeLookup | null) => void;
  onBlur?: () => void;
  isDisable?: boolean;
  placeholder?: string;
  label?: string;
...
export function EmployeeCombobox({
  ...
  const { data = [], isLoading } = useEmployeeLookup();
  const [filteredData, setFilteredData] = useState<IEmployeeLookup[]>(data);
  ...
  const resolvedValue = useMemo(() => {
    if (value == null || value.employeeId == null) return null;
    const id = Number(value.employeeId);
    if (!id) return null;
    const found = data.find((item) => Number(item.employeeId) === id);
    return found ?? value;
  }, [value, data]);
