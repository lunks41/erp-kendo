...
import type { IAccountSetupLookup } from "@/interfaces/lookup";

/**
 * Account Setup ComboBox with ERP-style filtering:
...
export function AccountSetupCombobox({
  value = null,
  onChange,
  onBlur,
  isDisable = false,
  placeholder = "Select account setup...",
  label,
  isRequired = false,
  error,
  dataItemKey = "accSetupId",
  textField = "accSetupName",
  allowCustom = false,
  fillMode = "outline",
  rounded = "medium",
  size = "medium",
  className,
  id,
}: AccountSetupComboboxProps) {
  const { data = [], isLoading } = useAccountSetupLookup();
  const [filteredData, setFilteredData] = useState<IAccountSetupLookup[]>(data);
  ...
  const resolvedValue = useMemo(() => {
    if (value == null || value.accSetupId == null) return null;
    const id = Number(value.accSetupId);
    if (!id) return null;
    const found = data.find((item) => Number(item.accSetupId) === id);
    return found ?? value;
  }, [value, data]);
