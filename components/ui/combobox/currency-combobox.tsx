...
/**
 * Currency ComboBox with ERP-style filtering:
...
export function CurrencyCombobox({
  ...
  const { data = [], isLoading } = useCurrencyLookup();
  const [filteredData, setFilteredData] = useState<ICurrencyLookup[]>(data);
  ...
  const resolvedValue = useMemo(() => {
    if (value == null || value.currencyId == null) return null;
    const id = Number(value.currencyId);
    if (!id) return null;
    const found = data.find((item) => Number(item.currencyId) === id);
    return found ?? value;
  }, [value, data]);
