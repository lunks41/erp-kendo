...
import type { IUserLookup } from "@/interfaces/lookup";

/**
 * User ComboBox with ERP-style filtering:
...
export interface UserComboboxProps {
  value?: IUserLookup | null;
  onChange?: (value: IUserLookup | null) => void;
  onBlur?: () => void;
  isDisable?: boolean;
  placeholder?: string;
  label?: string;
...
export function UserCombobox({
  ...
  const [filteredData, setFilteredData] = useState<IUserLookup[]>(data);
  ...
  const resolvedValue = useMemo(() => {
    ...
    if (!id) return null;
    const found = data.find((item) => Number(item.userId) === id);
    return found ?? value;
  }, [value, data]);
