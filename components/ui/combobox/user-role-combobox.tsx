...
import type { IUserRoleLookup } from "@/interfaces/lookup";

/**
 * User Role ComboBox with ERP-style filtering:
 ...
 */
export interface UserRoleComboboxProps {
  value?: IUserRoleLookup | null;
  onChange?: (value: IUserRoleLookup | null) => void;
  onBlur?: () => void;
  isDisable?: boolean;
  placeholder?: string;
  label?: string;
...
export function UserRoleCombobox({
  ...
  const { data = [], isLoading } = useUserRoleLookup();
  const [filteredData, setFilteredData] = useState<IUserRoleLookup[]>(data);
  ...
  const resolvedValue = useMemo(() => {
    if (value == null || value.userRoleId == null) return null;
    const id = Number(value.userRoleId);
    if (!id) return null;
    const found = data.find((item) => Number(item.userRoleId) === id);
    return found ?? value;
  }, [value, data]);
