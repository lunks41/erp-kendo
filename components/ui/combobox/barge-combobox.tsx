...
import type { IBargeLookup } from "@/interfaces/lookup";

/**
 * Barge ComboBox with ERP-style filtering:
...
export function BargeCombobox({
  ...
  const { data = [], isLoading } = useBargeLookup();
  const [filteredData, setFilteredData] = useState<IBargeLookup[]>(data);
  ...
  const resolvedValue = useMemo(() => {
    if (value == null || value.bargeId == null) return null;
    const id = Number(value.bargeId);
    if (!id) return null;
    const found = data.find((item) => Number(item.bargeId) === id);
    return found ?? value;
  }, [value, data]);
