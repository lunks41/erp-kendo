...
import type { ICategoryLookup } from "@/interfaces/lookup";

/**
 * Category ComboBox with ERP-style filtering:
...
export interface CategoryComboboxProps {
  value?: ICategoryLookup | null;
  onChange?: (value: ICategoryLookup | null) => void;
  onBlur?: () => void;
  isDisable?: boolean;
  placeholder?: string;
  label?: string;
...
export function CategoryCombobox({
  ...
  const { data = [], isLoading } = useCategoryLookup();
  const [filteredData, setFilteredData] = useState<ICategoryLookup[]>(data);
  ...
  const resolvedValue = useMemo(() => {
    if (value == null || value.categoryId == null) return null;
    const id = Number(value.categoryId);
    if (!id) return null;
    const found = data.find((item) => Number(item.categoryId) === id);
    return found ?? value;
  }, [value, data]);
