...
/**
 * GST ComboBox with ERP-style filtering:
 ...
 */
export interface GstComboboxProps {
  value?: IGstLookup | null;
  onChange?: (value: IGstLookup | null) => void;
  onBlur?: () => void;
  isDisable?: boolean;
  placeholder?: string;
  label?: string;
...
export function GstCombobox({
  ...
  const { data = [], isLoading } = useGstLookup();
  const [filteredData, setFilteredData] = useState<IGstLookup[]>(data);
  ...
  const resolvedValue = useMemo(() => {
    if (value == null || value.gstId == null) return null;
    const id = Number(value.gstId);
    if (!id) return null;
    const found = data.find((item) => Number(item.gstId) === id);
    return found ?? value;
  }, [value, data]);
