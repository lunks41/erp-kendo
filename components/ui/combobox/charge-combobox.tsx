...
/**
 * Charge ComboBox with ERP-style filtering:
 ...
 */
export interface ChargeComboboxProps {
  value?: IChargeLookup | null;
  onChange?: (value: IChargeLookup | null) => void;
  onBlur?: () => void;
  isDisable?: boolean;
  placeholder?: string;
  label?: string;
...
export function ChargeCombobox({
  value = null,
  onChange,
  onBlur,
  isDisable = false,
  ...
  const { data = [], isLoading } = useChargeLookup();
  const [filteredData, setFilteredData] = useState<IChargeLookup[]>(data);
  ...
  const resolvedValue = useMemo(() => {
    if (value == null || value.chargeId == null) return null;
    const id = Number(value.chargeId);
    if (!id) return null;
    const found = data.find((item) => Number(item.chargeId) === id);
    return found ?? value;
  }, [value, data]);
