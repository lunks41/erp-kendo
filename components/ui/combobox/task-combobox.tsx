...
/**
 * Task ComboBox with ERP-style filtering:
...
export interface TaskComboboxProps {
  value?: ITaskLookup | null;
  onChange?: (value: ITaskLookup | null) => void;
  onBlur?: () => void;
  isDisable?: boolean;
  placeholder?: string;
  label?: string;
...
export function TaskCombobox({
  ...
  const { data = [], isLoading } = useTaskLookup();
  const [filteredData, setFilteredData] = useState<ITaskLookup[]>(data);
  ...
  const resolvedValue = useMemo(() => {
    if (value == null || value.taskId == null) return null;
    const id = Number(value.taskId);
    if (!id) return null;
    const found = data.find((item) => Number(item.taskId) === id);
    return found ?? value;
  }, [value, data]);
