...
import type { IEmployerLookup } from "@/interfaces/lookup";

/**
 * Employer ComboBox with ERP-style filtering:
...
export interface EmployerComboboxProps {
  value?: IEmployerLookup | null;
  onChange?: (value: IEmployerLookup | null) => void;
  onBlur?: () => void;
  isDisable?: boolean;
  placeholder?: string;
  label?: string;
...
export function EmployerCombobox({
  ...
  const { data = [], isLoading } = useEmployerLookup();
  const [filteredData, setFilteredData] = useState<IEmployerLookup[]>(data);
  ...
  const resolvedValue = useMemo(() => {
    if (value == null || value.employerId == null) return null;
    const id = Number(value.employerId);
    if (!id) return null;
    const found = data.find((item) => Number(item.employerId) === id);
    return found ?? value;
  }, [value, data]);
