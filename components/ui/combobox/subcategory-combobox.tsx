...
import type { ISubCategoryLookup } from "@/interfaces/lookup";

/**
 * SubCategory ComboBox with ERP-style filtering:
...
export interface SubcategoryComboboxProps {
  value?: ISubCategoryLookup | null;
  onChange?: (value: ISubCategoryLookup | null) => void;
  onBlur?: () => void;
  isDisable?: boolean;
  placeholder?: string;
  label?: string;
...
export function SubcategoryCombobox({
  ...
  const { data = [], isLoading } = useSubCategoryLookup();
  const [filteredData, setFilteredData] = useState<ISubCategoryLookup[]>(data);
  ...
  const resolvedValue = useMemo(() => {
    if (value == null || value.subCategoryId == null) return null;
    const id = Number(value.subCategoryId);
    if (!id) return null;
    const found = data.find((item) => Number(item.subCategoryId) === id);
    return found ?? value;
  }, [value, data]);
