...
import type { IPortLookup } from "@/interfaces/lookup";

/**
 * Port ComboBox with ERP-style filtering:
...
export function PortCombobox({
  ...
  const { data = [], isLoading } = usePortLookup();
  const [filteredData, setFilteredData] = useState<IPortLookup[]>(data);
  ...
  const resolvedValue = useMemo(() => {
    if (value == null || value.portId == null) return null;
    const id = Number(value.portId);
    if (!id) return null;
    const found = data.find((item) => Number(item.portId) === id);
    return found ?? value;
  }, [value, data]);
