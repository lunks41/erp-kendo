...
/**
 * Credit Term ComboBox with ERP-style filtering:
...
export function CreditTermCombobox({
  value = null,
  onChange,
  onBlur,
  isDisable = false,
  ...
  className,
  id,
}: CreditTermComboboxProps) {
  const { data = [], isLoading } = useCreditTermLookup();
  const [filteredData, setFilteredData] = useState<ICreditTermLookup[]>(data);
  ...
  const resolvedValue = useMemo(() => {
    if (value == null || value.creditTermId == null) return null;
    const id = Number(value.creditTermId);
    if (!id) return null;
    const found = data.find((item) => Number(item.creditTermId) === id);
    return found ?? value;
  }, [value, data]);
