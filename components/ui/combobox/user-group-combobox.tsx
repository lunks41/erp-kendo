...
import type { IUserGroupLookup } from "@/interfaces/lookup";

/**
 * User Group ComboBox with ERP-style filtering:
 ...
 */
export interface UserGroupComboboxProps {
  value?: IUserGroupLookup | null;
  onChange?: (value: IUserGroupLookup | null) => void;
  onBlur?: () => void;
  isDisable?: boolean;
  placeholder?: string;
  label?: string;
...
export function UserGroupCombobox({
  value = null,
  onChange,
  onBlur,
  isDisable = false,
  ...
  const { data = [], isLoading } = useUserGroupLookup();
  const [filteredData, setFilteredData] = useState<IUserGroupLookup[]>(data);
  ...
  const resolvedValue = useMemo(() => {
    if (value == null || value.userGroupId == null) return null;
    const id = Number(value.userGroupId);
    if (!id) return null;
    const found = data.find((item) => Number(item.userGroupId) === id);
    return found ?? value;
  }, [value, data]);
