...
/**
 * Tax ComboBox with ERP-style filtering:
 ...
 */
export interface TaxComboboxProps {
  value?: ITaxLookup | null;
  onChange?: (value: ITaxLookup | null) => void;
  onBlur?: () => void;
  isDisable?: boolean;
  placeholder?: string;
  label?: string;
...
export function TaxCombobox({
  ...
  const { data = [], isLoading } = useTaxLookup();
  const [filteredData, setFilteredData] = useState<ITaxLookup[]>(data);
  ...
  const resolvedValue = useMemo(() => {
    if (value == null || value.taxId == null) return null;
    const id = Number(value.taxId);
    if (!id) return null;
    const found = data.find((item) => Number(item.taxId) === id);
    return found ?? value;
  }, [value, data]);
