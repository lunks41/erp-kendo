...
/**
 * Customer ComboBox with ERP-style filtering:
...
export function CustomerCombobox({
  ...
  const [filteredData, setFilteredData] = useState<ICustomerLookup[]>(data);
  ...
  const resolvedValue = useMemo(() => {
    if (value == null || value.customerId == null) return null;
    const id = Number(value.customerId);
    if (!id) return null;
    const found = data.find((item) => Number(item.customerId) === id);
    return found ?? value;
  }, [value, data]);
