...
import type { IServiceCategoryLookup } from "@/interfaces/lookup";

/**
 * Service Category ComboBox with ERP-style filtering:
 ...
 */
export interface ServiceCategoryComboboxProps {
  value?: IServiceCategoryLookup | null;
  onChange?: (value: IServiceCategoryLookup | null) => void;
  onBlur?: () => void;
  isDisable?: boolean;
  placeholder?: string;
  label?: string;
...
export function ServiceCategoryCombobox({
  ...
  const { data = [], isLoading } = useServiceCategoryLookup();
  const [filteredData, setFilteredData] = useState<IServiceCategoryLookup[]>(
    data
  );
  ...
  const resolvedValue = useMemo(() => {
    if (value == null || value.serviceCategoryId == null) return null;
    const id = Number(value.serviceCategoryId);
    if (!id) return null;
    const found = data.find((item) => Number(item.serviceCategoryId) === id);
    return found ?? value;
  }, [value, data]);
