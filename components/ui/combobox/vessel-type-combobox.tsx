...
/**
 * Vessel Type ComboBox with ERP-style filtering:
 ...
 */
export interface VesselTypeComboboxProps {
  /** Selected item or partial { vesselTypeId } for resolution from data */
  value?: IVesselTypeLookup | { vesselTypeId?: number } | null;
  onChange?: (value: IVesselTypeLookup | null) => void;
  onBlur?: () => void;
  isDisable?: boolean;
  placeholder?: string;
  label?: string;
...
export function VesselTypeCombobox({
  value = null,
  onChange,
  onBlur,
  isDisable = false,
  ...
  className,
  id,
}: VesselTypeComboboxProps) {
  const { data = [], isLoading } = useVesselTypeLookup();
  const [filteredData, setFilteredData] = useState<IVesselTypeLookup[]>(data);
  ...
  const resolvedValue = useMemo(() => {
    if (value == null || value.vesselTypeId == null) return null;
    const id = Number(value.vesselTypeId);
    if (!id) return null;
    const found = data.find((item) => Number(item.vesselTypeId) === id);
    return found ?? value;
  }, [value, data]);
