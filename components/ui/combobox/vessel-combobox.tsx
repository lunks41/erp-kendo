...
import type { IVesselLookup } from "@/interfaces/lookup";

/**
 * Vessel ComboBox with ERP-style filtering:
...
export function VesselCombobox({
  value = null,
  onChange,
  onBlur,
  isDisable = false,
  ...
  const { data = [], isLoading } = useVesselLookup();
  const [filteredData, setFilteredData] = useState<IVesselLookup[]>(data);
  ...
  const resolvedValue = useMemo(() => {
    if (value == null || value.vesselId == null) return null;
    const id = Number(value.vesselId);
    if (!id) return null;
    const found = data.find((item) => Number(item.vesselId) === id);
    return found ?? value;
  }, [value, data]);
