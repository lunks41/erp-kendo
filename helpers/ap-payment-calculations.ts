...
import { IApPaymentDt, IDecimal } from "@/interfaces"

// ============================================================================
// UTILITY FUNCTIONS
// ============================================================================

export const validateAllocation = (details: IApPaymentDt[]): boolean => {
...
export const autoAllocateAmounts = (
  ...
  if (totAmt === 0) {
    ...
    const byItemNo = new Map<number, IApPaymentDt>()
...
export const calculateManualAllocation = (
  ...
  totAmt?: number,
  decimals?: IDecimal
): { result: IApPaymentDt; wasAutoSetToZero: boolean } => {
  if (!details || rowNumber < 0 || rowNumber >= details.length) {
