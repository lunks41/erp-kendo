...
import { IArRefundDt, IDecimal } from "@/interfaces"

// ============================================================================
// UTILITY FUNCTIONS
// ============================================================================

export const validateAllocation = (details: IArRefundDt[]): boolean => {
...
export const autoAllocateAmounts = (
  ...
  if (totAmt === 0) {
    ...
    const byItemNo = new Map<number, IArRefundDt>()
...
export const calculateManualAllocation = (
  ...
  totAmt?: number,
  decimals?: IDecimal
): { result: IArRefundDt; wasAutoSetToZero: boolean } => {
  if (!details || rowNumber < 0 || rowNumber >= details.length) {
