...
import type { UserSettingSchemaType } from "@/schemas/setting";
import { userSettingSchema } from "@/schemas/setting";
...
export function AccountForm() {
  ...
  useEffect(() => {
    if (
      ...
      const res = userSettingResponse as UserSettingResponse;
      if (res.result === -2) {
  ...
  const handleConfirmSave = () => {
    ...
    saveUserSettings(data, {
      onSuccess: (response) => {
        const r = response as UserSettingResponse;
        if (r.result === -2) {
          toast.error("This record is locked");
          return;
        }
        if (r.result === -1) {
          toast.error(r.message || "Failed to save user settings");
          return;
        }
        if (r.result === 1) {
          toast.success(r.message || "User settings saved successfully");
          refetch();
        }
      },
      onError: (err) => {
        toast.error(err instanceof Error ? err.message : "Failed to save user settings");
      },
    });
  };
  ...
  if (isError) {
    return (
      <div className="flex flex-col items-center justify-center gap-4 p-8">
        <p className="text-rose-500">Failed to load user settings</p>
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-muted-foreground text-sm">
                Configure grid records and GL account mappings for your account
              </p>
            </div>
