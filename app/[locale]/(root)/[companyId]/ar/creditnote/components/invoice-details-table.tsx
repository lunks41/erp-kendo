...
export default function InvoiceDetailsTable({
  ...
  const columns = [
    ...
    ...(visible?.m_GstId ? [
      ...
      { field: "gstPercentage", title: "GST %", cell: ({ dataItem }: { dataItem: IArInvoiceDt }) => <div className="text-right">{formatNumber(dataItem.gstPercentage, 2)}</div> },
      { field: "gstAmt", title: "GST Amount", cell: ({ dataItem }: { dataItem: IArInvoiceDt }) => <div className="text-right">{formatNumber(dataItem.gstAmt ?? 0, amtDec)}</div> },
      { field: "gstLocalAmt", title: "GST Local", cell: ({ dataItem }: { dataItem: IArInvoiceDt }) => <div className="text-right">{formatNumber(dataItem.gstLocalAmt ?? 0, locAmtDec)}</div> },
    ] : []),
