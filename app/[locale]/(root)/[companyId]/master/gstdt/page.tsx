...
import { GstDt } from "@/lib/api-routes";
...
export default function GstDtMasterPage() {
  const t = useNamespaceTranslations("gstDt");
  const tc = useNamespaceTranslations("common");
  const { decimals } = useAuthStore();
  const moduleId = ModuleId.master;
  const transactionId = MasterTransactionId.gstDt;
  ...
  const [itemToDelete, setItemToDelete] = useState<IGstDt | null>(null);
  const [saveDialogOpen, setSaveDialogOpen] = useState(false);
  const [pendingSaveData, setPendingSaveData] =
    useState<Partial<IGstDt> | null>(null);
  const [selectedItem, setSelectedItem] = useState<IGstDt | null>(null);
  const [viewMode, setViewMode] = useState(false);
  const [currentPage, setCurrentPage] = useState(1);
  const [pageSize, setPageSize] = useState<number | null>(null);
  const effectivePageSize =
    pageSize ?? (defaults?.common?.masterGridTotalRecords || 50);
  const { data: response, isLoading } = useGetWithPagination<IGstDt>(
    `${GstDt.get}`,
    "gstDt",
    searchFilter,
    currentPage,
  ...
  const saveMutation = usePersist<IGstDt>(GstDt.add);
  const deleteMutation = useDelete(GstDt.delete);
  const invalidateList = useCallback(
    () => queryClient.invalidateQueries({ queryKey: ["gstDt"] }),
    [queryClient],
  );
  ...
  const handleView = useCallback((item: IGstDt) => {
    setSelectedItem(item);
    setViewMode(true);
    setDialogOpen(true);
  }, []);
  const handleEdit = useCallback((item: IGstDt) => {
    setSelectedItem(item);
    setViewMode(false);
    setDialogOpen(true);
  }, []);
  const handleDelete = useCallback((item: IGstDt) => {
  ...
  const handleDeleteConfirm = useCallback(async () => {
    ...
    try {
      await deleteMutation.mutateAsync(String(itemToDelete.gstId));
      await invalidateList();
      setDeleteDialogOpen(false);
      setItemToDelete(null);
    } catch {}
  }, [itemToDelete, deleteMutation, invalidateList]);
  const handleFormSubmit = useCallback((payload: Partial<IGstDt>) => {
  ...
  return (
    <div className="flex flex-col gap-4 p-4">
      ...
      {dialogOpen && (
        ...
        >
          {viewMode && selectedItem ? (
              ...
              <div className="grid grid-cols-2 gap-4 text-sm">
                <div>
                  ...
                  <p className="font-medium">
                    {selectedItem.gstCode} - {selectedItem.gstName}
                  </p>
                </div>
                <div>
                  <span className="text-slate-500 dark:text-slate-400">
                  ...
                  <p className="font-medium">{selectedItem.gstPercentage}</p>
                </div>
                <div>
                  <span className="text-slate-500 dark:text-slate-400">
                    {t("validFrom")}
            ...
            <GstDtForm
              key={selectedItem ? `${selectedItem.gstId}-${selectedItem.validFrom}` : "new"}
      ...
      <ConfirmationDialog
        ...
        type="save"
        title={selectedItem ? t("updateGstDt") : t("createGstDt")}
        message={
          selectedItem
            ? `Update GST "${selectedItem.gstName}" detail?`
            : "Create this GST detail?"
        }
      ...
      <ConfirmationDialog
        ...
        message={
          itemToDelete
            ? `Delete GST "${itemToDelete.gstName}" detail?`
            : "Delete this item?"
        }
