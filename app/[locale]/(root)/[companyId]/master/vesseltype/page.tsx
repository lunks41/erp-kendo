...
import { VesselTypeTable } from "./components/vesseltype-table";
import { useNamespaceTranslations } from "@/hooks/use-form-translations";

export default function VesselTypeMasterPage() {
  ...
  const [itemToDelete, setItemToDelete] = useState<IVesselType | null>(null);
  const [saveDialogOpen, setSaveDialogOpen] = useState(false);
  const [pendingSaveData, setPendingSaveData] =
    useState<Partial<IVesselType> | null>(null);
  const [selectedItem, setSelectedItem] = useState<IVesselType | null>(null);
  const [viewMode, setViewMode] = useState(false);
  const [currentPage, setCurrentPage] = useState(1);
  const [pageSize, setPageSize] = useState<number | null>(null);
  const effectivePageSize =
    pageSize ?? (defaults?.common?.masterGridTotalRecords || 50);
  const { data: response, isLoading } = useGetWithPagination<IVesselType>(
    `${VesselType.get}`,
    "vesselTypes",
    searchFilter,
    currentPage,
  ...
  const saveMutation = usePersist<IVesselType>(VesselType.add);
  ...
  const handleView = useCallback((item: IVesselType) => {
    setSelectedItem(item);
    setViewMode(true);
    setDialogOpen(true);
  }, []);
  const handleEdit = useCallback((item: IVesselType) => {
    setSelectedItem(item);
    setViewMode(false);
    setDialogOpen(true);
  }, []);
  const handleDelete = useCallback((item: IVesselType) => {
  ...
  const handleDeleteConfirm = useCallback(async () => {
    ...
    try {
      await deleteMutation.mutateAsync(String(itemToDelete.vesselTypeId));
      await invalidateList();
      setDeleteDialogOpen(false);
      setItemToDelete(null);
    } catch {}
  }, [itemToDelete, deleteMutation, invalidateList]);
  const handleFormSubmit = useCallback((payload: Partial<IVesselType>) => {
  ...
  return (
    <div className="flex flex-col gap-4 p-4">
      ...
      {dialogOpen && (
        ...
        >
          {viewMode && selectedItem ? (
            <div className="space-y-4 py-2">
              <div className="grid grid-cols-2 gap-4 text-sm">
                <div>
                  ...
                  <p className="font-medium">{selectedItem.vesselTypeCode}</p>
                </div>
                <div>
                  <span className="text-slate-500 dark:text-slate-400">
                    Name
            ...
            <VesselTypeForm
              key={selectedItem?.vesselTypeId ?? "new"}
      ...
      <ConfirmationDialog
        ...
        type="save"
        title={selectedItem ? t("updateVesselType") : t("createVesselType")}
        message={
          selectedItem
            ? `Update "${selectedItem.vesselTypeName}"?`
            : "Create this vessel type?"
        }
      ...
      <ConfirmationDialog
        ...
        type="delete"
        title={t("deleteVesselType")}
        message={
          itemToDelete
            ? `Delete "${itemToDelete.vesselTypeName}"?`
            : "Delete this item?"
        }
