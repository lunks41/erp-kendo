...
import type { ISupplyType } from "@/interfaces/supplytype";
...
import { SupplyTypeTable } from "./components/supplytype-table";
import { useNamespaceTranslations } from "@/hooks/use-form-translations";

export default function SupplyTypeMasterPage() {
  const t = useNamespaceTranslations("supplyType");
  ...
  const [itemToDelete, setItemToDelete] = useState<ISupplyType | null>(null);
  const [saveDialogOpen, setSaveDialogOpen] = useState(false);
  const [pendingSaveData, setPendingSaveData] =
    useState<Partial<ISupplyType> | null>(null);
  const [selectedItem, setSelectedItem] = useState<ISupplyType | null>(null);
  const [viewMode, setViewMode] = useState(false);
  const [currentPage, setCurrentPage] = useState(1);
  const [pageSize, setPageSize] = useState<number | null>(null);
  const effectivePageSize =
    pageSize ?? (defaults?.common?.masterGridTotalRecords || 50);
  const { data: response, isLoading } = useGetWithPagination<ISupplyType>(
    `${SupplyType.get}`,
    "supplyTypes",
    searchFilter,
    currentPage,
  ...
  const saveMutation = usePersist<ISupplyType>(SupplyType.add);
  ...
  const handleView = useCallback((item: ISupplyType) => {
    setSelectedItem(item);
    setViewMode(true);
    setDialogOpen(true);
  }, []);
  const handleEdit = useCallback((item: ISupplyType) => {
    setSelectedItem(item);
    setViewMode(false);
    setDialogOpen(true);
  }, []);
  const handleDelete = useCallback((item: ISupplyType) => {
  ...
  const handleDeleteConfirm = useCallback(async () => {
    ...
    try {
      await deleteMutation.mutateAsync(String(itemToDelete.supplyTypeId));
      await invalidateList();
      setDeleteDialogOpen(false);
      setItemToDelete(null);
    } catch {}
  }, [itemToDelete, deleteMutation, invalidateList]);
  const handleFormSubmit = useCallback((payload: Partial<ISupplyType>) => {
  ...
  return (
    <div className="flex flex-col gap-4 p-4">
      ...
      {dialogOpen && (
        ...
        >
          {viewMode && selectedItem ? (
            <div className="space-y-4 py-2">
              <div className="grid grid-cols-2 gap-4 text-sm">
                <div>
                  ...
                  <p className="font-medium">{selectedItem.supplyTypeCode}</p>
                </div>
                <div>
                  <span className="text-slate-500 dark:text-slate-400">
                    {t("name")}
                  </span>
                  <p className="font-medium">{selectedItem.supplyTypeName}</p>
                </div>
            ...
            <SupplyTypeForm
              key={selectedItem?.supplyTypeId ?? "new"}
      ...
      <ConfirmationDialog
        ...
        type="save"
        title={selectedItem ? t("updateSupplyType") : t("createSupplyType")}
        message={
          selectedItem
            ? `Update "${selectedItem.supplyTypeName}"?`
            : "Create this supply type?"
        }
      ...
      <ConfirmationDialog
        ...
        type="delete"
        title={t("deleteSupplyType")}
        message={
          itemToDelete
            ? `Delete "${itemToDelete.supplyTypeName}"?`
            : "Delete this item?"
        }
