...
import { AccountSetupCategory } from "@/lib/api-routes";
import type { IAccountSetupCategory } from "@/interfaces/accountsetup";
import { ConfirmationDialog } from "@/components/ui/confirmation";
import { TableSkeleton } from "@/components/skeleton";
import { AccountSetupCategoryForm } from "./components/accountsetupcategory-form";
import { MasterTransactionId, ModuleId } from "@/lib/utils";
import { usePermissionStore } from "@/stores/permission-store";
import { useUserSettingDefaults } from "@/hooks/use-settings";
import { AccountSetupCategoryTable } from "./components/accountsetupcategory-table";
import { useNamespaceTranslations } from "@/hooks/use-form-translations";

export default function AccountSetupCategoryMasterPage() {
  const t = useNamespaceTranslations("accountSetupCategory");
  const tc = useNamespaceTranslations("common");
  const moduleId = ModuleId.master;
  const transactionId = MasterTransactionId.accountSetupCategory;
  ...
  const [pendingSaveData, setPendingSaveData] =
    useState<Partial<IAccountSetupCategory> | null>(null);
  ...
  const saveMutation = usePersist<IAccountSetupCategory>(
    AccountSetupCategory.add
  );
  const deleteMutation = useDelete(AccountSetupCategory.delete);
  const invalidateList = useCallback(
    () => queryClient.invalidateQueries({ queryKey: ["accountSetupCategories"] }),
    [queryClient]
  );
  ...
  const handleView = useCallback((item: IAccountSetupCategory) => {
    setSelectedItem(item);
    setViewMode(true);
    setDialogOpen(true);
  }, []);
  const handleEdit = useCallback((item: IAccountSetupCategory) => {
    setSelectedItem(item);
    setViewMode(false);
    setDialogOpen(true);
  }, []);
  const handleDelete = useCallback((item: IAccountSetupCategory) => {
  ...
  const handleDeleteConfirm = useCallback(async () => {
    ...
    try {
      await deleteMutation.mutateAsync(
        String(itemToDelete.accSetupCategoryId)
      );
  ...
  const handleFormSubmit = useCallback(
    (payload: Partial<IAccountSetupCategory>) => {
  ...
  return (
    <div className="flex flex-col gap-4 p-4">
      ...
      {dialogOpen && (
        <Dialog
          title={
            selectedItem
              ? viewMode
                ? t("viewAccountSetupCategory")
                : t("editAccountSetupCategory")
              : t("createAccountSetupCategory")
          }
        ...
        >
          {viewMode && selectedItem ? (
            <div className="space-y-4 py-2">
              <div className="grid grid-cols-2 gap-4 text-sm">
                <div>
                  ...
                  <p className="font-medium">{selectedItem.accSetupCategoryCode}</p>
                </div>
                <div>
                  <span className="text-slate-500 dark:text-slate-400">
                    {tc("name")}
                  </span>
                  <p className="font-medium">{selectedItem.accSetupCategoryName}</p>
                </div>
            ...
            <AccountSetupCategoryForm
              key={selectedItem?.accSetupCategoryId ?? "new"}
      ...
      <ConfirmationDialog
        ...
        type="save"
        title={
          selectedItem ? t("updateAccountSetupCategory") : t("createAccountSetupCategory")
        }
        message={
          selectedItem
            ? `Update "${selectedItem.accSetupCategoryName}"?`
            : "Create this item?"
        }
      ...
      <ConfirmationDialog
        ...
        type="delete"
        title={t("deleteAccountSetupCategory")}
        message={
          itemToDelete
            ? `Delete "${itemToDelete.accSetupCategoryName}"?`
            : "Delete this item?"
        }
