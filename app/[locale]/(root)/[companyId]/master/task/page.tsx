...
export default function TaskMasterPage() {
  ...
  const [pendingSaveData, setPendingSaveData] = useState<Partial<ITask> | null>(
    null,
  );
  ...
  const { data: response, isLoading } = useGetWithPagination<ITask>(
    `${Task.get}`,
    "tasks",
    searchFilter,
    currentPage,
  ...
  const saveMutation = usePersist<ITask>(Task.add);
  ...
  const handleView = useCallback((item: ITask) => {
    setSelectedItem(item);
    setViewMode(true);
    setDialogOpen(true);
  }, []);
  const handleEdit = useCallback((item: ITask) => {
    setSelectedItem(item);
    setViewMode(false);
    setDialogOpen(true);
  }, []);
  const handleDelete = useCallback((item: ITask) => {
  ...
  const handleDeleteConfirm = useCallback(async () => {
    ...
    try {
      await deleteMutation.mutateAsync(String(itemToDelete.taskId));
      await invalidateList();
      setDeleteDialogOpen(false);
      setItemToDelete(null);
    } catch {}
  }, [itemToDelete, deleteMutation, invalidateList]);
  const handleFormSubmit = useCallback((payload: Partial<ITask>) => {
  ...
  return (
    <div className="flex flex-col gap-4 p-4">
            ...
            <div className="space-y-4 py-2">
              <div className="grid grid-cols-2 gap-4 text-sm">
                <div>
                  ...
                  <p className="font-medium">{selectedItem.taskCode}</p>
                </div>
                <div>
                  <span className="text-slate-500 dark:text-slate-400">
                    Name
                  </span>
                  <p className="font-medium">{selectedItem.taskName}</p>
                </div>
                <div>
                  <span className="text-slate-500 dark:text-slate-400">
                    Task Order
                  </span>
                  <p className="font-medium">{selectedItem.taskOrder}</p>
                </div>
      ...
      <ConfirmationDialog
        ...
        type="save"
        title={selectedItem ? t("updateTask") : t("createTask")}
        message={
          selectedItem
            ? `Update "${selectedItem.taskName}"?`
            : "Create this task?"
        }
