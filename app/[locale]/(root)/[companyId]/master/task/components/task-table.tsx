...
function TaskTableInner(props: TaskTableProps) {
  ...
  const defaultColumns: MasterDataGridColumn[] = useMemo(
    () => [
      { field: "taskId", title: "Id", width: 80, hidden: true },
      { field: "taskCode", title: tc("code"), width: 100, minWidth: 80 },
      { field: "taskName", title: tc("name"), flex: true, minWidth: 150 },
      {
        field: "isActive",
        title: tc("active"),
        width: 100,
        ...
        cells: {
          data: (props) => {
            const isActive = (props.dataItem as ITask).isActive;
        ...
        cells: {
          data: (p) => (
            <td {...p.tdProps} className="k-table-td whitespace-nowrap">
              {formatDateTime((p.dataItem as ITask).createDate, datetimeFormat)}
            </td>
          ),
        },
        media: "(min-width: 992px)",
      ...
      },
      {
        ...
        cells: {
          data: (p) => (
            <td {...p.tdProps} className="k-table-td whitespace-nowrap">
              {formatDateTime((p.dataItem as ITask).editDate, datetimeFormat)}
            </td>
          ),
        },
        media: "(min-width: 1200px)",
  ...
  return (
    <MasterDataGrid
      ...
      dataItemKey="taskId"
      actions={{
        onView: (i) => onView(i as unknown as ITask),
        onEdit: (i) => onEdit(i as unknown as ITask),
        onDelete: (i) => onDelete(i as unknown as ITask),
      }}
      pageable
      groupable={false}
      pageSize={pageSize}
...
export const TaskTable = memo(TaskTableInner);
