...
import { ServiceType } from "@/lib/api-routes";
...
export default function ServiceTypeMasterPage() {
  ...
  const [itemToDelete, setItemToDelete] = useState<IServiceType | null>(null);
  const [saveDialogOpen, setSaveDialogOpen] = useState(false);
  const [pendingSaveData, setPendingSaveData] =
    useState<Partial<IServiceType> | null>(null);
  ...
  const { data: response, isLoading } = useGetWithPagination<IServiceType>(
    `${ServiceType.get}`,
    "serviceTypes",
    searchFilter,
    currentPage,
  ...
  const saveMutation = usePersist<IServiceType>(ServiceType.add);
  ...
  const handleView = useCallback((item: IServiceType) => {
    setSelectedItem(item);
    setViewMode(true);
    setDialogOpen(true);
  }, []);
  const handleEdit = useCallback((item: IServiceType) => {
    setSelectedItem(item);
    setViewMode(false);
    setDialogOpen(true);
  }, []);
  const handleDelete = useCallback((item: IServiceType) => {
  ...
  const handleDeleteConfirm = useCallback(async () => {
    ...
    try {
      await deleteMutation.mutateAsync(String(itemToDelete.serviceTypeId));
      await invalidateList();
      setDeleteDialogOpen(false);
      setItemToDelete(null);
    } catch {}
  }, [itemToDelete, deleteMutation, invalidateList]);
  const handleFormSubmit = useCallback((payload: Partial<IServiceType>) => {
  ...
  return (
    <div className="flex flex-col gap-4 p-4">
      ...
      {dialogOpen && (
        ...
        >
          {viewMode && selectedItem ? (
            <div className="space-y-4 py-2">
              <div className="grid grid-cols-2 gap-4 text-sm">
                <div>
                  ...
                  <p className="font-medium">{selectedItem.serviceTypeCode}</p>
                </div>
                <div>
                  <span className="text-slate-500 dark:text-slate-400">
                    {tc("name")}
                  </span>
                  <p className="font-medium">{selectedItem.serviceTypeName}</p>
                </div>
                <div>
                  <span className="text-slate-500 dark:text-slate-400">
                    {t("serviceTypeCategoryId")}
                  </span>
                  <p className="font-medium">
                    {selectedItem.serviceTypeCategoryId}
                  </p>
                </div>
            ...
            <ServiceTypeForm
              key={selectedItem?.serviceTypeId ?? "new"}
      ...
      <ConfirmationDialog
        ...
        type="save"
        title={selectedItem ? t("updateServiceType") : t("createServiceType")}
        message={
          selectedItem
            ? `Update "${selectedItem.serviceTypeName}"?`
            : "Create this service type?"
        }
      ...
      <ConfirmationDialog
        ...
        type="delete"
        title={t("deleteServiceType")}
        message={
          itemToDelete
            ? `Delete "${itemToDelete.serviceTypeName}"?`
            : "Delete this item?"
        }
