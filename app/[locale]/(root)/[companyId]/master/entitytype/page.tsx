...
import { EntityTypeForm } from "./components/entitytype-form";
import { MasterTransactionId, ModuleId } from "@/lib/utils";
import { usePermissionStore } from "@/stores/permission-store";
import { useUserSettingDefaults } from "@/hooks/use-settings";
import { EntityTypeTable } from "./components/entitytype-table";
import { useNamespaceTranslations } from "@/hooks/use-form-translations";

export default function EntityTypeMasterPage() {
  ...
  const [itemToDelete, setItemToDelete] = useState<IEntityType | null>(null);
  const [saveDialogOpen, setSaveDialogOpen] = useState(false);
  const [pendingSaveData, setPendingSaveData] =
    useState<Partial<IEntityType> | null>(null);
  const [selectedItem, setSelectedItem] = useState<IEntityType | null>(null);
  const [viewMode, setViewMode] = useState(false);
  const [currentPage, setCurrentPage] = useState(1);
  const [pageSize, setPageSize] = useState<number | null>(null);
  const effectivePageSize =
    pageSize ?? (defaults?.common?.masterGridTotalRecords || 50);
  const { data: response, isLoading } = useGetWithPagination<IEntityType>(
    `${EntityType.get}`,
    "entityTypes",
    searchFilter,
    currentPage,
  ...
  const saveMutation = usePersist<IEntityType>(EntityType.add);
  ...
  const handleView = useCallback((item: IEntityType) => {
    setSelectedItem(item);
    setViewMode(true);
    setDialogOpen(true);
  }, []);
  const handleEdit = useCallback((item: IEntityType) => {
    setSelectedItem(item);
    setViewMode(false);
    setDialogOpen(true);
  }, []);
  const handleDelete = useCallback((item: IEntityType) => {
  ...
  const handleDeleteConfirm = useCallback(async () => {
    ...
    try {
      await deleteMutation.mutateAsync(String(itemToDelete.entityTypeId));
      await invalidateList();
      setDeleteDialogOpen(false);
      setItemToDelete(null);
    } catch {}
  }, [itemToDelete, deleteMutation, invalidateList]);
  const handleFormSubmit = useCallback((payload: Partial<IEntityType>) => {
  ...
  return (
    <div className="flex flex-col gap-4 p-4">
      ...
      {dialogOpen && (
        ...
        >
          {viewMode && selectedItem ? (
            <div className="space-y-4 py-2">
              <div className="grid grid-cols-2 gap-4 text-sm">
                <div>
                  ...
                  <p className="font-medium">{selectedItem.entityTypeCode}</p>
                </div>
                <div>
                  <span className="text-slate-500 dark:text-slate-400">
                    Name
                  </span>
                  <p className="font-medium">{selectedItem.entityTypeName}</p>
                </div>
              </div>
              <DialogActionsBar>
            ...
            <EntityTypeForm
              key={selectedItem?.entityTypeId ?? "new"}
      ...
      <ConfirmationDialog
        ...
        type="save"
        title={selectedItem ? t("updateEntityType") : t("createEntityType")}
        message={
          selectedItem
            ? `Update "${selectedItem.entityTypeName}"?`
      ...
      <ConfirmationDialog
        ...
        type="delete"
        title={t("deleteEntityType")}
        message={
          itemToDelete
            ? `Delete "${itemToDelete.entityTypeName}"?`
            : "Delete this item?"
        }
