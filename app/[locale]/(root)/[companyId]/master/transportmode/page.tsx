...
import { TransportModeForm } from "./components/transportmode-form";
import { MasterTransactionId, ModuleId } from "@/lib/utils";
import { usePermissionStore } from "@/stores/permission-store";
import { useUserSettingDefaults } from "@/hooks/use-settings";
import { TransportModeTable } from "./components/transportmode-table";
import { useNamespaceTranslations } from "@/hooks/use-form-translations";

export default function TransportModeMasterPage() {
  ...
  const [itemToDelete, setItemToDelete] = useState<ITransportMode | null>(null);
  const [saveDialogOpen, setSaveDialogOpen] = useState(false);
  const [pendingSaveData, setPendingSaveData] =
    useState<Partial<ITransportMode> | null>(null);
  const [selectedItem, setSelectedItem] = useState<ITransportMode | null>(null);
  const [viewMode, setViewMode] = useState(false);
  const [currentPage, setCurrentPage] = useState(1);
  const [pageSize, setPageSize] = useState<number | null>(null);
  const effectivePageSize =
    pageSize ?? (defaults?.common?.masterGridTotalRecords || 50);
  const { data: response, isLoading } = useGetWithPagination<ITransportMode>(
    `${TransportMode.get}`,
    "transportModes",
    searchFilter,
    currentPage,
  ...
  const saveMutation = usePersist<ITransportMode>(TransportMode.add);
  ...
  const handleView = useCallback((item: ITransportMode) => {
    setSelectedItem(item);
    setViewMode(true);
    setDialogOpen(true);
  }, []);
  const handleEdit = useCallback((item: ITransportMode) => {
    setSelectedItem(item);
    setViewMode(false);
    setDialogOpen(true);
  }, []);
  const handleDelete = useCallback((item: ITransportMode) => {
  ...
  const handleDeleteConfirm = useCallback(async () => {
    ...
    try {
      await deleteMutation.mutateAsync(String(itemToDelete.transportModeId));
      await invalidateList();
      setDeleteDialogOpen(false);
      setItemToDelete(null);
    } catch {}
  }, [itemToDelete, deleteMutation, invalidateList]);
  const handleFormSubmit = useCallback((payload: Partial<ITransportMode>) => {
  ...
  return (
    <div className="flex flex-col gap-4 p-4">
      ...
      {dialogOpen && (
        ...
        >
          {viewMode && selectedItem ? (
              ...
              <div className="grid grid-cols-2 gap-4 text-sm">
                <div>
                  ...
                  <p className="font-medium">
                    {selectedItem.transportModeCode}
                  </p>
                </div>
                <div>
                  <span className="text-slate-500 dark:text-slate-400">
                  ...
                  <p className="font-medium">
                    {selectedItem.transportModeName}
                  </p>
                </div>
                <div>
                  <span className="text-slate-500 dark:text-slate-400">
            ...
            <TransportModeForm
              key={selectedItem?.transportModeId ?? "new"}
      ...
      <ConfirmationDialog
        ...
        type="save"
        title={
          selectedItem ? t("updateTransportMode") : t("createTransportMode")
        }
        message={
          selectedItem
            ? `Update "${selectedItem.transportModeName}"?`
            : "Create this transport mode?"
        }
      ...
      <ConfirmationDialog
        ...
        message={
          itemToDelete
            ? `Delete "${itemToDelete.transportModeName}"?`
            : "Delete this item?"
        }
