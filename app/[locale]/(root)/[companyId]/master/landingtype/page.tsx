...
import { LandingTypeForm } from "./components/landingtype-form";
import { MasterTransactionId, ModuleId } from "@/lib/utils";
import { usePermissionStore } from "@/stores/permission-store";
import { useUserSettingDefaults } from "@/hooks/use-settings";
import { LandingTypeTable } from "./components/landingtype-table";
import { useNamespaceTranslations } from "@/hooks/use-form-translations";

export default function LandingTypeMasterPage() {
  ...
  const [itemToDelete, setItemToDelete] = useState<ILandingType | null>(null);
  const [saveDialogOpen, setSaveDialogOpen] = useState(false);
  const [pendingSaveData, setPendingSaveData] =
    useState<Partial<ILandingType> | null>(null);
  const [selectedItem, setSelectedItem] = useState<ILandingType | null>(null);
  const [viewMode, setViewMode] = useState(false);
  const [currentPage, setCurrentPage] = useState(1);
  const [pageSize, setPageSize] = useState<number | null>(null);
  const effectivePageSize =
    pageSize ?? (defaults?.common?.masterGridTotalRecords || 50);
  const { data: response, isLoading } = useGetWithPagination<ILandingType>(
    `${LandingType.get}`,
    "landingTypes",
    searchFilter,
    currentPage,
  ...
  const saveMutation = usePersist<ILandingType>(LandingType.add);
  ...
  const handleView = useCallback((item: ILandingType) => {
    setSelectedItem(item);
    setViewMode(true);
    setDialogOpen(true);
  }, []);
  const handleEdit = useCallback((item: ILandingType) => {
    setSelectedItem(item);
    setViewMode(false);
    setDialogOpen(true);
  }, []);
  const handleDelete = useCallback((item: ILandingType) => {
  ...
  const handleDeleteConfirm = useCallback(async () => {
    ...
    try {
      await deleteMutation.mutateAsync(String(itemToDelete.landingTypeId));
      await invalidateList();
      setDeleteDialogOpen(false);
      setItemToDelete(null);
    } catch {}
  }, [itemToDelete, deleteMutation, invalidateList]);
  const handleFormSubmit = useCallback((payload: Partial<ILandingType>) => {
    ...
    <div className="flex flex-col gap-4 p-4">
      <div>
        <h1 className="flex items-center gap-1.5 text-lg font-semibold text-slate-900 dark:text-white">
          <Anchor className="h-5 w-5 text-rose-500" />
          {t("title")}
        </h1>
      ...
      {dialogOpen && (
        ...
        >
          {viewMode && selectedItem ? (
            <div className="space-y-4 py-2">
              <div className="grid grid-cols-2 gap-4 text-sm">
                <div>
                  ...
                  <p className="font-medium">{selectedItem.landingTypeCode}</p>
                </div>
                <div>
                  <span className="text-slate-500 dark:text-slate-400">
                    Name
                  </span>
                  <p className="font-medium">{selectedItem.landingTypeName}</p>
                </div>
                <div>
                  <span className="text-slate-500 dark:text-slate-400">
                    Seq No
            ...
            <LandingTypeForm
              key={selectedItem?.landingTypeId ?? "new"}
      ...
      <ConfirmationDialog
        ...
        type="save"
        title={selectedItem ? "Update Landing Type" : "Create Landing Type"}
        message={
          selectedItem
            ? `Update "${selectedItem.landingTypeName}"?`
            : "Create this landing type?"
        }
      ...
      <ConfirmationDialog
        ...
        message={
          itemToDelete
            ? `Delete "${itemToDelete.landingTypeName}"?`
            : "Delete this item?"
        }
