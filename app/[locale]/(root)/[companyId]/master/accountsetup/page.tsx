...
import { AccountSetup } from "@/lib/api-routes";
...
import { AccountSetupForm } from "./components/accountsetup-form";
import { AccountSetupTable } from "./components/accountsetup-table";
import { useNamespaceTranslations } from "@/hooks/use-form-translations";

export default function AccountSetupMasterPage() {
  ...
  const [itemToDelete, setItemToDelete] = useState<IAccountSetup | null>(null);
  const [saveDialogOpen, setSaveDialogOpen] = useState(false);
  const [pendingSaveData, setPendingSaveData] =
    useState<Partial<IAccountSetup> | null>(null);
  const [selectedItem, setSelectedItem] = useState<IAccountSetup | null>(null);
  ...
  const { data: response, isLoading } = useGetWithPagination<IAccountSetup>(
    `${AccountSetup.get}`,
    "accountSetups",
    searchFilter,
    currentPage,
  ...
  const saveMutation = usePersist<IAccountSetup>(AccountSetup.add);
  const deleteMutation = useDelete(AccountSetup.delete);
  ...
  const handleView = useCallback((item: IAccountSetup) => {
    setSelectedItem(item);
    setViewMode(true);
    setDialogOpen(true);
  }, []);
  const handleEdit = useCallback((item: IAccountSetup) => {
    setSelectedItem(item);
    setViewMode(false);
    setDialogOpen(true);
  }, []);
  const handleDelete = useCallback((item: IAccountSetup) => {
  ...
  const handleDeleteConfirm = useCallback(async () => {
    ...
    try {
      await deleteMutation.mutateAsync(String(itemToDelete.accSetupId));
      await invalidateList();
      setDeleteDialogOpen(false);
      setItemToDelete(null);
    } catch {}
  }, [itemToDelete, deleteMutation, invalidateList]);
  const handleFormSubmit = useCallback((payload: Partial<IAccountSetup>) => {
  ...
  return (
    <div className="flex flex-col gap-4 p-4">
      ...
      {dialogOpen && (
        ...
        >
          {viewMode && selectedItem ? (
            <div className="space-y-4 py-2">
              <div className="grid grid-cols-2 gap-4 text-sm">
                <div>
                  ...
                  <p className="font-medium">{selectedItem.accSetupCode}</p>
                </div>
                <div>
                  <span className="text-slate-500 dark:text-slate-400">
                    Name
            ...
            <AccountSetupForm
              key={selectedItem?.accSetupId ?? "new"}
      ...
      <ConfirmationDialog
        ...
        type="save"
        title={selectedItem ? "Update Account Setup" : "Create Account Setup"}
        message={
          selectedItem
            ? `Update "${selectedItem.accSetupName}"?`
            : "Create this account setup?"
        }
      ...
      <ConfirmationDialog
        ...
        type="delete"
        title="Delete Account Setup"
        message={
          itemToDelete
            ? `Delete "${itemToDelete.accSetupName}"?`
            : "Delete this item?"
        }
