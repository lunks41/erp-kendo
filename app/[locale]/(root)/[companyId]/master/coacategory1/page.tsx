...
import { CoaCategory1 } from "@/lib/api-routes";
...
export default function CoaCategory1MasterPage() {
  const t = useNamespaceTranslations("coaCategory1");
  const tc = useNamespaceTranslations("common");
  const moduleId = ModuleId.master;
  const transactionId = MasterTransactionId.coaCategory1;
  ...
  const [itemToDelete, setItemToDelete] = useState<ICoaCategory1 | null>(null);
  const [saveDialogOpen, setSaveDialogOpen] = useState(false);
  const [pendingSaveData, setPendingSaveData] =
    useState<Partial<ICoaCategory1> | null>(null);
  const [selectedItem, setSelectedItem] = useState<ICoaCategory1 | null>(null);
  const [viewMode, setViewMode] = useState(false);
  const [currentPage, setCurrentPage] = useState(1);
  const [pageSize, setPageSize] = useState<number | null>(null);
  const effectivePageSize =
    pageSize ?? (defaults?.common?.masterGridTotalRecords || 50);
  const { data: response, isLoading } = useGetWithPagination<ICoaCategory1>(
    `${CoaCategory1.get}`,
    "coaCategory1",
    searchFilter,
    currentPage,
  ...
  const saveMutation = usePersist<ICoaCategory1>(CoaCategory1.add);
  const deleteMutation = useDelete(CoaCategory1.delete);
  ...
  const handleView = useCallback((item: ICoaCategory1) => {
    setSelectedItem(item);
    setViewMode(true);
    setDialogOpen(true);
  }, []);
  const handleEdit = useCallback((item: ICoaCategory1) => {
    setSelectedItem(item);
    setViewMode(false);
    setDialogOpen(true);
  }, []);
  const handleDelete = useCallback((item: ICoaCategory1) => {
  ...
  const handleFormSubmit = useCallback((payload: Partial<ICoaCategory1>) => {
  ...
  return (
    <div className="flex flex-col gap-4 p-4">
      ...
      <ConfirmationDialog
        ...
        type="save"
        title={selectedItem ? t("updateCoaCategory1") : t("createCoaCategory1")}
        message={
          selectedItem
            ? `Update "${selectedItem.coaCategoryName}"?`
            : "Create this COA category 1?"
        }
