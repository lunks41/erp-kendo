...
export default function ProductMasterPage() {
  ...
  const { data: response, isLoading } = useGetWithPagination<IProduct>(
    `${Product.get}`,
    "products",
    searchFilter,
    currentPage,
  ...
  const saveMutation = usePersist<IProduct>(Product.add);
  ...
  const handleView = useCallback((item: IProduct) => {
    setSelectedItem(item);
    setViewMode(true);
    setDialogOpen(true);
  }, []);
  const handleEdit = useCallback((item: IProduct) => {
    setSelectedItem(item);
    setViewMode(false);
    setDialogOpen(true);
  }, []);
  const handleDelete = useCallback((item: IProduct) => {
  ...
  const handleDeleteConfirm = useCallback(async () => {
    ...
    try {
      await deleteMutation.mutateAsync(String(itemToDelete.productId));
      await invalidateList();
      setDeleteDialogOpen(false);
      setItemToDelete(null);
    } catch {}
  }, [itemToDelete, deleteMutation, invalidateList]);
  const handleFormSubmit = useCallback((payload: Partial<IProduct>) => {
  ...
  return (
    <div className="flex flex-col gap-4 p-4">
      ...
      {dialogOpen && (
        ...
        >
          {viewMode && selectedItem ? (
            <div className="space-y-4 py-2">
              <div className="grid grid-cols-2 gap-4 text-sm">
                <div>
                  ...
                  <p className="font-medium">{selectedItem.productCode}</p>
                </div>
                <div>
                  <span className="text-slate-500 dark:text-slate-400">
                    Name
            ...
            <ProductForm
              key={selectedItem?.productId ?? "new"}
      ...
      <ConfirmationDialog
        ...
        type="save"
        title={selectedItem ? "Update Product" : "Create Product"}
        message={
          selectedItem
            ? `Update "${selectedItem.productName}"?`
            : "Create this product?"
        }
      ...
      <ConfirmationDialog
        ...
        message={
          itemToDelete
            ? `Delete "${itemToDelete.productName}"?`
            : "Delete this item?"
        }
