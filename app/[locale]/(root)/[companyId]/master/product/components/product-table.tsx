...
function ProductTableInner(props: ProductTableProps) {
  ...
  const defaultColumns: MasterDataGridColumn[] = useMemo(
    () => [
      { field: "productId", title: "Id", width: 80, hidden: true },
      { field: "productCode", title: tc("code"), width: 100, minWidth: 80 },
      { field: "productName", title: tc("name"), flex: true, minWidth: 150 },
      {
        field: "isActive",
        title: tc("active"),
        width: 100,
        cells: {
          data: (props) => {
            const isActive = (props.dataItem as IProduct).isActive;
        ...
        cells: {
          data: (p) => (
            <td {...p.tdProps} className="k-table-td whitespace-nowrap">
              {formatDateTime(
                (p.dataItem as IProduct).createDate,
                datetimeFormat,
              )}
            </td>
          ),
      ...
      },
      {
        ...
        cells: {
          data: (p) => (
            <td {...p.tdProps} className="k-table-td whitespace-nowrap">
              {formatDateTime(
                (p.dataItem as IProduct).editDate,
                datetimeFormat,
              )}
            </td>
          ),
  ...
  return (
    <MasterDataGrid
      ...
      actions={{
        onView: (i) => onView(i as unknown as IProduct),
        onEdit: (i) => onEdit(i as unknown as IProduct),
        onDelete: (i) => onDelete(i as unknown as IProduct),
      }}
      pageable
      groupable={false}
      pageSize={pageSize}
...
export const ProductTable = memo(ProductTableInner);
