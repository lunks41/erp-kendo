...
import { DocumentTypeForm } from "./components/documenttype-form";
import { MasterTransactionId, ModuleId } from "@/lib/utils";
import { usePermissionStore } from "@/stores/permission-store";
import { useUserSettingDefaults } from "@/hooks/use-settings";
import { DocumentTypeTable } from "./components/documenttype-table";
...
export default function DocumentTypeMasterPage() {
  ...
  const documentTypePermission = getPermissions(moduleId, transactionId);
  const hasNoRights =
    mounted && permissionsLoaded && documentTypePermission === undefined;

  const [searchFilter, setSearchFilter] = useState("");
  const [searchInput, setSearchInput] = useState("");
  const [dialogOpen, setDialogOpen] = useState(false);
  const [deleteDialogOpen, setDeleteDialogOpen] = useState(false);
  const [itemToDelete, setItemToDelete] = useState<IDocumentType | null>(null);
  const [saveDialogOpen, setSaveDialogOpen] = useState(false);
  const [pendingSaveData, setPendingSaveData] =
    useState<Partial<IDocumentType> | null>(null);
  const [selectedItem, setSelectedItem] = useState<IDocumentType | null>(null);
  ...
  const { data: response, isLoading } = useGetWithPagination<IDocumentType>(
    `${DocumentType.get}`,
    "documentTypes",
    searchFilter,
    currentPage,
  ...
  const saveMutation = usePersist<IDocumentType>(DocumentType.add);
  ...
  const handleView = useCallback((item: IDocumentType) => {
    setSelectedItem(item);
    setViewMode(true);
    setDialogOpen(true);
  }, []);

  const handleEdit = useCallback((item: IDocumentType) => {
    setSelectedItem(item);
    setViewMode(false);
    setDialogOpen(true);
  }, []);

  const handleDelete = useCallback((item: IDocumentType) => {
  ...
  const handleDeleteConfirm = useCallback(async () => {
    ...
    try {
      await deleteMutation.mutateAsync(String(itemToDelete.documentTypeId));
  ...
  const handleFormSubmit = useCallback((payload: Partial<IDocumentType>) => {
  ...
  return (
    <div className="flex flex-col gap-4 p-4">
            ...
            <div className="space-y-4 py-2">
              <div className="grid grid-cols-2 gap-4 text-sm">
                <div>
                  ...
                  <p className="font-medium">{selectedItem.documentTypeCode}</p>
                </div>
                <div>
                  <span className="text-slate-500 dark:text-slate-400">
                    Name
                  </span>
                  <p className="font-medium">{selectedItem.documentTypeName}</p>
                </div>
      ...
      <ConfirmationDialog
        ...
        type="save"
        title={selectedItem ? "Update Document Type" : "Create Document Type"}
        message={
          selectedItem
            ? `Are you sure you want to update document type "${selectedItem.documentTypeName}"?`
            : "Are you sure you want to create this document type?"
        }
      ...
      <ConfirmationDialog
        ...
        message={
          itemToDelete
            ? `Are you sure you want to delete document type "${itemToDelete.documentTypeName}"? This action cannot be undone.`
