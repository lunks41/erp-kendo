...
export interface TaxTableProps {
  data: ITax[];
  totalRecords?: number;
  onView: (item: ITax) => void;
  onEdit: (item: ITax) => void;
  onDelete: (item: ITax) => void;
  onAdd?: () => void;
  onRefresh?: () => void;
  searchFilter?: string;
...
function TaxTableInner(props: TaxTableProps) {
  ...
  const defaultColumns: MasterDataGridColumn[] = useMemo(
    () => [
      { field: "taxId", title: "Id", width: 80, hidden: true },
      { field: "taxCode", title: tc("code"), width: 100, minWidth: 80 },
      { field: "taxName", title: tc("name"), flex: true, minWidth: 150 },
      {
        field: "isActive",
        title: tc("active"),
        width: 100,
        ...
        cells: {
          data: (props) => {
            const isActive = (props.dataItem as ITax).isActive;
        ...
        cells: {
          data: (p) => (
            <td {...p.tdProps} className="k-table-td whitespace-nowrap">
              {formatDateTime((p.dataItem as ITax).createDate, datetimeFormat)}
            </td>
          ),
        },
        media: "(min-width: 992px)",
      ...
      },
      {
        ...
        cells: {
          data: (p) => (
            <td {...p.tdProps} className="k-table-td whitespace-nowrap">
              {formatDateTime((p.dataItem as ITax).editDate, datetimeFormat)}
            </td>
          ),
        },
        media: "(min-width: 1200px)",
  ...
  return (
    <MasterDataGrid
      ...
      dataItemKey="taxId"
      actions={{
        onView: (i) => onView(i as unknown as ITax),
        onEdit: (i) => onEdit(i as unknown as ITax),
        onDelete: (i) => onDelete(i as unknown as ITax),
      }}
      pageable
      groupable={false}
      pageSize={pageSize}
...
export const TaxTable = memo(TaxTableInner);
