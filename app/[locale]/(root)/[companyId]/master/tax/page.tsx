...
import { Tax } from "@/lib/api-routes";
...
export default function TaxMasterPage() {
  const t = useNamespaceTranslations("tax");
  ...
  const [itemToDelete, setItemToDelete] = useState<ITax | null>(null);
  const [saveDialogOpen, setSaveDialogOpen] = useState(false);
  const [pendingSaveData, setPendingSaveData] = useState<Partial<ITax> | null>(
    null,
  );
  const [selectedItem, setSelectedItem] = useState<ITax | null>(null);
  const [viewMode, setViewMode] = useState(false);
  const [currentPage, setCurrentPage] = useState(1);
  const [pageSize, setPageSize] = useState<number | null>(null);
  const effectivePageSize =
    pageSize ?? (defaults?.common?.masterGridTotalRecords || 50);
  const { data: response, isLoading } = useGetWithPagination<ITax>(
    `${Tax.get}`,
    "taxes",
    searchFilter,
    currentPage,
  ...
  const saveMutation = usePersist<ITax>(Tax.add);
  ...
  const handleView = useCallback((item: ITax) => {
    setSelectedItem(item);
    setViewMode(true);
    setDialogOpen(true);
  }, []);
  const handleEdit = useCallback((item: ITax) => {
    setSelectedItem(item);
    setViewMode(false);
    setDialogOpen(true);
  }, []);
  const handleDelete = useCallback((item: ITax) => {
  ...
  const handleFormSubmit = useCallback((payload: Partial<ITax>) => {
  ...
  return (
    <div className="flex flex-col gap-4 p-4">
      ...
      {dialogOpen && (
        <Dialog
          title={
            selectedItem ? (viewMode ? "View Tax" : "Edit Tax") : "Create Tax"
          }
            ...
            <div className="space-y-4 py-2">
              <div className="grid grid-cols-2 gap-4 text-sm">
                <div>
                  ...
                  <p className="font-medium">{selectedItem.taxCode}</p>
                </div>
                <div>
                  <span className="text-slate-500 dark:text-slate-400">
                    Name
                  </span>
                  <p className="font-medium">{selectedItem.taxName}</p>
                </div>
                <div>
                  <span className="text-slate-500 dark:text-slate-400">
                    Tax Category Id
                  </span>
                  <p className="font-medium">{selectedItem.taxCategoryId}</p>
                </div>
      ...
      <ConfirmationDialog
        ...
        type="save"
        title={selectedItem ? t("updateTax") : t("createTax")}
        message={
          selectedItem
            ? `Update "${selectedItem.taxName}"?`
            : "Create this tax?"
        }
