...
export default function VisaMasterPage() {
  ...
  const [itemToDelete, setItemToDelete] = useState<IVisa | null>(null);
  const [saveDialogOpen, setSaveDialogOpen] = useState(false);
  const [pendingSaveData, setPendingSaveData] = useState<Partial<IVisa> | null>(
    null,
  );
  const [selectedItem, setSelectedItem] = useState<IVisa | null>(null);
  const [viewMode, setViewMode] = useState(false);
  const [currentPage, setCurrentPage] = useState(1);
  const [pageSize, setPageSize] = useState<number | null>(null);
  const effectivePageSize =
    pageSize ?? (defaults?.common?.masterGridTotalRecords || 50);
  const { data: response, isLoading } = useGetWithPagination<IVisa>(
    `${Visa.get}`,
    "visas",
    searchFilter,
    currentPage,
  ...
  const saveMutation = usePersist<IVisa>(Visa.add);
  ...
  const handleView = useCallback((item: IVisa) => {
    setSelectedItem(item);
    setViewMode(true);
    setDialogOpen(true);
  }, []);
  const handleEdit = useCallback((item: IVisa) => {
    setSelectedItem(item);
    setViewMode(false);
    setDialogOpen(true);
  }, []);
  const handleDelete = useCallback((item: IVisa) => {
  ...
  const handleDeleteConfirm = useCallback(async () => {
    ...
    try {
      await deleteMutation.mutateAsync(String(itemToDelete.visaId));
      await invalidateList();
      setDeleteDialogOpen(false);
      setItemToDelete(null);
    } catch {}
  }, [itemToDelete, deleteMutation, invalidateList]);
  const handleFormSubmit = useCallback((payload: Partial<IVisa>) => {
  ...
  return (
    <div className="flex flex-col gap-4 p-4">
            ...
            <div className="space-y-4 py-2">
              <div className="grid grid-cols-2 gap-4 text-sm">
                <div>
                  ...
                  <p className="font-medium">{selectedItem.visaCode}</p>
                </div>
                <div>
                  <span className="text-slate-500 dark:text-slate-400">
                    Name
                  </span>
                  <p className="font-medium">{selectedItem.visaName}</p>
                </div>
                <div>
                  <span className="text-slate-500 dark:text-slate-400">
                    Seq No
      ...
      <ConfirmationDialog
        ...
        type="save"
        title={selectedItem ? t("updateVisa") : t("createVisa")}
        message={
          selectedItem
            ? `Update "${selectedItem.visaName}"?`
            : "Create this visa?"
        }
