...
import type { IVisa } from "@/interfaces/visa";
...
export interface VisaTableProps {
  data: IVisa[];
  totalRecords?: number;
  onView: (item: IVisa) => void;
  onEdit: (item: IVisa) => void;
  onDelete: (item: IVisa) => void;
  onAdd?: () => void;
  onRefresh?: () => void;
  searchFilter?: string;
...
function VisaTableInner(props: VisaTableProps) {
  const t = useNamespaceTranslations("visa");
    ...
    () => [
        ...
        cells: {
          data: (p) => (
            <td {...p.tdProps} className="k-table-td whitespace-nowrap">
              {formatDateTime((p.dataItem as IVisa).createDate, datetimeFormat)}
            </td>
          ),
        },
        media: "(min-width: 992px)",
      ...
      },
      {
        ...
        cells: {
          data: (p) => (
            <td {...p.tdProps} className="k-table-td whitespace-nowrap">
              {formatDateTime((p.dataItem as IVisa).editDate, datetimeFormat)}
            </td>
          ),
        },
        media: "(min-width: 1200px)",
  ...
  return (
    <MasterDataGrid
      ...
      dataItemKey="visaId"
      actions={{
        onView: (i) => onView(i as unknown as IVisa),
        onEdit: (i) => onEdit(i as unknown as IVisa),
        onDelete: (i) => onDelete(i as unknown as IVisa),
      }}
      pageable
      groupable={false}
      pageSize={pageSize}
...
export const VisaTable = memo(VisaTableInner);
