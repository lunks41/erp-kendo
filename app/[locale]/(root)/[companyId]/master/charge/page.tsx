...
export default function ChargeMasterPage() {
  ...
  const [itemToDelete, setItemToDelete] = useState<ICharge | null>(null);
  const [saveDialogOpen, setSaveDialogOpen] = useState(false);
  const [pendingSaveData, setPendingSaveData] =
    useState<Partial<ICharge> | null>(null);
  const [selectedItem, setSelectedItem] = useState<ICharge | null>(null);
  const [viewMode, setViewMode] = useState(false);
  const [currentPage, setCurrentPage] = useState(1);
  const [pageSize, setPageSize] = useState<number | null>(null);
  const effectivePageSize =
    pageSize ?? (defaults?.common?.masterGridTotalRecords || 50);
  const { data: response, isLoading } = useGetWithPagination<ICharge>(
    `${Charge.get}`,
    "charges",
    searchFilter,
    currentPage,
  ...
  const saveMutation = usePersist<ICharge>(Charge.add);
  ...
  const handleView = useCallback((item: ICharge) => {
    setSelectedItem(item);
    setViewMode(true);
    setDialogOpen(true);
  }, []);
  const handleEdit = useCallback((item: ICharge) => {
    setSelectedItem(item);
    setViewMode(false);
    setDialogOpen(true);
  }, []);
  const handleDelete = useCallback((item: ICharge) => {
  ...
  const handleDeleteConfirm = useCallback(async () => {
    ...
    try {
      await deleteMutation.mutateAsync(String(itemToDelete.chargeId));
      await invalidateList();
      setDeleteDialogOpen(false);
      setItemToDelete(null);
    } catch {}
  }, [itemToDelete, deleteMutation, invalidateList]);
  const handleFormSubmit = useCallback((payload: Partial<ICharge>) => {
  ...
  return (
    <div className="flex flex-col gap-4 p-4">
            ...
            <div className="space-y-4 py-2">
              <div className="grid grid-cols-2 gap-4 text-sm">
                <div>
                  ...
                  <p className="font-medium">{selectedItem.chargeCode}</p>
                </div>
                <div>
                  <span className="text-slate-500 dark:text-slate-400">
                    Name
      ...
      <ConfirmationDialog
        ...
        type="save"
        title={selectedItem ? "Update Charge" : "Create Charge"}
        message={
          selectedItem
            ? `Update "${selectedItem.chargeName}"?`
            : "Create this charge?"
        }
      ...
      <ConfirmationDialog
        ...
        message={
          itemToDelete
            ? `Delete "${itemToDelete.chargeName}"?`
            : "Delete this item?"
        }
