...
export interface ChargeTableProps {
  data: ICharge[];
  totalRecords?: number;
  onView: (item: ICharge) => void;
  onEdit: (item: ICharge) => void;
  onDelete: (item: ICharge) => void;
  onAdd?: () => void;
  onRefresh?: () => void;
  searchFilter?: string;
...
function ChargeTableInner(props: ChargeTableProps) {
  ...
  const defaultColumns: MasterDataGridColumn[] = useMemo(
    () => [
      { field: "chargeId", title: "Id", width: 80, hidden: true },
      { field: "chargeCode", title: tc("code"), width: 100, minWidth: 80 },
      { field: "chargeName", title: tc("name"), flex: true, minWidth: 150 },
      {
        field: "isActive",
        title: tc("active"),
        width: 100,
        cells: {
          data: (props) => {
            const isActive = (props.dataItem as ICharge).isActive;
        ...
        cells: {
          data: (p) => (
            <td {...p.tdProps} className="k-table-td whitespace-nowrap">
              {formatDateTime(
                (p.dataItem as ICharge).createDate,
                datetimeFormat,
              )}
            </td>
          ),
      ...
      },
      {
        ...
        cells: {
          data: (p) => (
            <td {...p.tdProps} className="k-table-td whitespace-nowrap">
              {formatDateTime((p.dataItem as ICharge).editDate, datetimeFormat)}
            </td>
          ),
        },
        media: "(min-width: 1200px)",
  ...
  return (
    <MasterDataGrid
      ...
      dataItemKey="chargeId"
      actions={{
        onView: (i) => onView(i as unknown as ICharge),
        onEdit: (i) => onEdit(i as unknown as ICharge),
        onDelete: (i) => onDelete(i as unknown as ICharge),
      }}
      pageable
      groupable={false}
      pageSize={pageSize}
...
export const ChargeTable = memo(ChargeTableInner);
