...
import { LeaveTypeTable } from "./components/leavetype-table";
...
export default function LeaveTypeMasterPage() {
  ...
  const [itemToDelete, setItemToDelete] = useState<ILeaveType | null>(null);
  const [saveDialogOpen, setSaveDialogOpen] = useState(false);
  const [pendingSaveData, setPendingSaveData] =
    useState<Partial<ILeaveType> | null>(null);
  const [selectedItem, setSelectedItem] = useState<ILeaveType | null>(null);
  const [viewMode, setViewMode] = useState(false);
  const [currentPage, setCurrentPage] = useState(1);
  const [pageSize, setPageSize] = useState<number | null>(null);
  const effectivePageSize =
    pageSize ?? (defaults?.common?.masterGridTotalRecords || 50);
  const { data: response, isLoading } = useGetWithPagination<ILeaveType>(
    `${LeaveType.get}`,
    "leaveTypes",
    searchFilter,
    currentPage,
  ...
  const saveMutation = usePersist<ILeaveType>(LeaveType.add);
  ...
  const handleView = useCallback((item: ILeaveType) => {
    setSelectedItem(item);
    setViewMode(true);
    setDialogOpen(true);
  }, []);
  const handleEdit = useCallback((item: ILeaveType) => {
    setSelectedItem(item);
    setViewMode(false);
    setDialogOpen(true);
  }, []);
  const handleDelete = useCallback((item: ILeaveType) => {
  ...
  const handleDeleteConfirm = useCallback(async () => {
    ...
    try {
      await deleteMutation.mutateAsync(String(itemToDelete.leaveTypeId));
      await invalidateList();
      setDeleteDialogOpen(false);
      setItemToDelete(null);
    } catch {}
  }, [itemToDelete, deleteMutation, invalidateList]);
  const handleFormSubmit = useCallback((payload: Partial<ILeaveType>) => {
  ...
  return (
    <div className="flex flex-col gap-4 p-4">
      ...
      {dialogOpen && (
        ...
        >
          {viewMode && selectedItem ? (
            <div className="space-y-4 py-2">
              <div className="grid grid-cols-2 gap-4 text-sm">
                <div>
                  ...
                  <p className="font-medium">{selectedItem.leaveTypeCode}</p>
                </div>
                <div>
                  <span className="text-slate-500 dark:text-slate-400">
                    Name
                  </span>
                  <p className="font-medium">{selectedItem.leaveTypeName}</p>
                </div>
            ...
            <LeaveTypeForm
              key={selectedItem?.leaveTypeId ?? "new"}
      ...
      <ConfirmationDialog
        ...
        type="save"
        title={selectedItem ? "Update Leave Type" : "Create Leave Type"}
        message={
          selectedItem
            ? `Update "${selectedItem.leaveTypeName}"?`
            : "Create this leave type?"
        }
      ...
      <ConfirmationDialog
        ...
        message={
          itemToDelete
            ? `Delete "${itemToDelete.leaveTypeName}"?`
            : "Delete this item?"
        }
