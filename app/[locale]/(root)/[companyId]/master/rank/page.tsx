...
import type { IRank } from "@/interfaces/rank";
...
export default function RankMasterPage() {
  ...
  const [itemToDelete, setItemToDelete] = useState<IRank | null>(null);
  const [saveDialogOpen, setSaveDialogOpen] = useState(false);
  const [pendingSaveData, setPendingSaveData] = useState<Partial<IRank> | null>(
    null,
  );
  const [selectedItem, setSelectedItem] = useState<IRank | null>(null);
  const [viewMode, setViewMode] = useState(false);
  const [currentPage, setCurrentPage] = useState(1);
  const [pageSize, setPageSize] = useState<number | null>(null);
  const effectivePageSize =
    pageSize ?? (defaults?.common?.masterGridTotalRecords || 50);
  const { data: response, isLoading } = useGetWithPagination<IRank>(
    `${Rank.get}`,
    "ranks",
    searchFilter,
    currentPage,
  ...
  const saveMutation = usePersist<IRank>(Rank.add);
  ...
  const handleView = useCallback((item: IRank) => {
    setSelectedItem(item);
    setViewMode(true);
    setDialogOpen(true);
  }, []);
  const handleEdit = useCallback((item: IRank) => {
    setSelectedItem(item);
    setViewMode(false);
    setDialogOpen(true);
  }, []);
  const handleDelete = useCallback((item: IRank) => {
  ...
  const handleDeleteConfirm = useCallback(async () => {
    ...
    try {
      await deleteMutation.mutateAsync(String(itemToDelete.rankId));
      await invalidateList();
      setDeleteDialogOpen(false);
      setItemToDelete(null);
    } catch {}
  }, [itemToDelete, deleteMutation, invalidateList]);
  const handleFormSubmit = useCallback((payload: Partial<IRank>) => {
  ...
  return (
    <div className="flex flex-col gap-4 p-4">
            ...
            <div className="space-y-4 py-2">
              <div className="grid grid-cols-2 gap-4 text-sm">
                <div>
                  ...
                  <p className="font-medium">{selectedItem.rankCode}</p>
                </div>
                <div>
                  <span className="text-slate-500 dark:text-slate-400">
                    {t("name")}
                  </span>
                  <p className="font-medium">{selectedItem.rankName}</p>
                </div>
                <div>
                  <span className="text-slate-500 dark:text-slate-400">
                    {tc("seqNo")}
      ...
      <ConfirmationDialog
        ...
        type="save"
        title={selectedItem ? t("updateRank") : t("createRank")}
        message={
          selectedItem
            ? `Update "${selectedItem.rankName}"?`
            : "Create this rank?"
        }
