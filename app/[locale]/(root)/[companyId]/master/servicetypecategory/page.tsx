...
import type { IServiceTypeCategory } from "@/interfaces/servicetype";
import { ServiceTypeCategory } from "@/lib/api-routes";
...
export default function ServiceTypeCategoryMasterPage() {
  const t = useNamespaceTranslations("serviceTypeCategory");
  const tc = useNamespaceTranslations("common");
  const moduleId = ModuleId.master;
  const transactionId = MasterTransactionId.serviceTypeCategory;
  ...
  const [pendingSaveData, setPendingSaveData] =
    useState<Partial<IServiceTypeCategory> | null>(null);
  ...
  const saveMutation = usePersist<IServiceTypeCategory>(
    ServiceTypeCategory.add,
  );
  const deleteMutation = useDelete(ServiceTypeCategory.delete);
  ...
  const handleView = useCallback((item: IServiceTypeCategory) => {
    setSelectedItem(item);
    setViewMode(true);
    setDialogOpen(true);
  }, []);
  const handleEdit = useCallback((item: IServiceTypeCategory) => {
    setSelectedItem(item);
    setViewMode(false);
    setDialogOpen(true);
  }, []);
  const handleDelete = useCallback((item: IServiceTypeCategory) => {
  ...
  const handleDeleteConfirm = useCallback(async () => {
    ...
    try {
      await deleteMutation.mutateAsync(
        String(itemToDelete.serviceTypeCategoryId),
      );
  ...
  const handleFormSubmit = useCallback(
    (payload: Partial<IServiceTypeCategory>) => {
  ...
  return (
    <div className="flex flex-col gap-4 p-4">
      ...
      {dialogOpen && (
        <Dialog
          title={
            selectedItem
              ? viewMode
                ? t("viewServiceTypeCategory")
                : t("editServiceTypeCategory")
              : t("createServiceTypeCategory")
          }
        ...
        >
          {viewMode && selectedItem ? (
              ...
              <div className="grid grid-cols-2 gap-4 text-sm">
                <div>
                  <span className="text-slate-500 dark:text-slate-400">
                    {t("serviceTypeCategoryCode")}
                  </span>
                  <p className="font-medium">
                    {selectedItem.serviceTypeCategoryCode}
                  </p>
                </div>
                <div>
                  <span className="text-slate-500 dark:text-slate-400">
                    {t("serviceTypeCategoryName")}
                  </span>
                  <p className="font-medium">
                    {selectedItem.serviceTypeCategoryName}
                  </p>
                </div>
            ...
            <ServiceTypeCategoryForm
              key={selectedItem?.serviceTypeCategoryId ?? "new"}
      ...
      <ConfirmationDialog
        ...
        title={
          selectedItem
            ? t("updateServiceTypeCategory")
            : t("createServiceTypeCategory")
        }
        message={
          selectedItem
            ? `Update "${selectedItem.serviceTypeCategoryName}"?`
            : "Create this service type category?"
        }
      ...
      <ConfirmationDialog
        ...
        type="delete"
        title={t("deleteServiceTypeCategory")}
        message={
          itemToDelete
            ? `Delete "${itemToDelete.serviceTypeCategoryName}"?`
            : "Delete this item?"
        }
