...
import { LoanTypeForm } from "./components/loantype-form";
import { MasterTransactionId, ModuleId } from "@/lib/utils";
import { usePermissionStore } from "@/stores/permission-store";
import { useUserSettingDefaults } from "@/hooks/use-settings";
import { LoanTypeTable } from "./components/loantype-table";
import { useNamespaceTranslations } from "@/hooks/use-form-translations";

export default function LoanTypeMasterPage() {
  ...
  const [itemToDelete, setItemToDelete] = useState<ILoanType | null>(null);
  const [saveDialogOpen, setSaveDialogOpen] = useState(false);
  const [pendingSaveData, setPendingSaveData] =
    useState<Partial<ILoanType> | null>(null);
  const [selectedItem, setSelectedItem] = useState<ILoanType | null>(null);
  const [viewMode, setViewMode] = useState(false);
  const [currentPage, setCurrentPage] = useState(1);
  const [pageSize, setPageSize] = useState<number | null>(null);
  const effectivePageSize =
    pageSize ?? (defaults?.common?.masterGridTotalRecords || 50);
  const { data: response, isLoading } = useGetWithPagination<ILoanType>(
    `${LoanType.get}`,
    "loanTypes",
    searchFilter,
    currentPage,
  ...
  const saveMutation = usePersist<ILoanType>(LoanType.add);
  ...
  const handleView = useCallback((item: ILoanType) => {
    setSelectedItem(item);
    setViewMode(true);
    setDialogOpen(true);
  }, []);
  const handleEdit = useCallback((item: ILoanType) => {
    setSelectedItem(item);
    setViewMode(false);
    setDialogOpen(true);
  }, []);
  const handleDelete = useCallback((item: ILoanType) => {
  ...
  const handleDeleteConfirm = useCallback(async () => {
    ...
    try {
      await deleteMutation.mutateAsync(String(itemToDelete.loanTypeId));
      await invalidateList();
      setDeleteDialogOpen(false);
      setItemToDelete(null);
    } catch {}
  }, [itemToDelete, deleteMutation, invalidateList]);
  const handleFormSubmit = useCallback((payload: Partial<ILoanType>) => {
  ...
  return (
    <div className="flex flex-col gap-4 p-4">
      ...
      {dialogOpen && (
        ...
        >
          {viewMode && selectedItem ? (
            <div className="space-y-4 py-2">
              <div className="grid grid-cols-2 gap-4 text-sm">
                <div>
                  ...
                  <p className="font-medium">{selectedItem.loanTypeCode}</p>
                </div>
                <div>
                  <span className="text-slate-500 dark:text-slate-400">
                    Name
                  </span>
                  <p className="font-medium">{selectedItem.loanTypeName}</p>
                </div>
                <div>
                  <span className="text-slate-500 dark:text-slate-400">
                    Interest Rate %
                  </span>
                  <p className="font-medium">{selectedItem.interestRatePct}</p>
                </div>
                <div>
                  <span className="text-slate-500 dark:text-slate-400">
                    Min Term (Months)
                  </span>
                  <p className="font-medium">{selectedItem.minTermMonths}</p>
                </div>
                <div>
                  <span className="text-slate-500 dark:text-slate-400">
                    Max Term (Months)
                  </span>
                  <p className="font-medium">{selectedItem.maxTermMonths}</p>
                </div>
              </div>
              <DialogActionsBar>
            ...
            <LoanTypeForm
              key={selectedItem?.loanTypeId ?? "new"}
      ...
      <ConfirmationDialog
        ...
        type="save"
        title={selectedItem ? "Update Loan Type" : "Create Loan Type"}
        message={
          selectedItem
            ? `Update "${selectedItem.loanTypeName}"?`
            : "Create this loan type?"
        }
      ...
      <ConfirmationDialog
        ...
        message={
          itemToDelete
            ? `Delete "${itemToDelete.loanTypeName}"?`
            : "Delete this item?"
        }
