...
export interface BargeTableProps {
  data: IBarge[];
  totalRecords?: number;
  onView: (item: IBarge) => void;
  onEdit: (item: IBarge) => void;
  onDelete: (item: IBarge) => void;
  onAdd?: () => void;
  onRefresh?: () => void;
  searchFilter?: string;
...
function BargeTableInner(props: BargeTableProps) {
  ...
  const defaultColumns: MasterDataGridColumn[] = useMemo(
    () => [
      { field: "bargeId", title: "Id", width: 80, hidden: true },
      { field: "bargeCode", title: tc("code"), width: 100, minWidth: 80 },
      { field: "bargeName", title: tc("name"), flex: true, minWidth: 150 },
      {
        field: "isActive",
        title: tc("active"),
        width: 100,
        cells: {
          data: (props) => {
            const isActive = (props.dataItem as IBarge).isActive;
        ...
        cells: {
          data: (props) => (
            <td {...props.tdProps} className="k-table-td whitespace-nowrap">
              {formatDateTime(
                (props.dataItem as IBarge).createDate,
                datetimeFormat,
              )}
            </td>
          ),
      ...
      },
      {
        ...
        cells: {
          data: (props) => (
            <td {...props.tdProps} className="k-table-td whitespace-nowrap">
              {formatDateTime(
                (props.dataItem as IBarge).editDate,
                datetimeFormat,
              )}
            </td>
          ),
  ...
  return (
    <MasterDataGrid
      ...
      dataItemKey="bargeId"
      actions={{
        onView: (item) => onView(item as unknown as IBarge),
        onEdit: (item) => onEdit(item as unknown as IBarge),
        onDelete: (item) => onDelete(item as unknown as IBarge),
      }}
      pageable
      groupable={false}
      pageSize={pageSize}
...
export const BargeTable = memo(BargeTableInner);
