...
export default function BargeMasterPage() {
  ...
  const [itemToDelete, setItemToDelete] = useState<IBarge | null>(null);
  const [saveDialogOpen, setSaveDialogOpen] = useState(false);
  const [pendingSaveData, setPendingSaveData] =
    useState<Partial<IBarge> | null>(null);
  const [selectedItem, setSelectedItem] = useState<IBarge | null>(null);
  ...
  const { data: response, isLoading } = useGetWithPagination<IBarge>(
    `${Barge.get}`,
    "barges",
    searchFilter,
    currentPage,
  ...
  const saveMutation = usePersist<IBarge>(Barge.add);
  ...
  const handleView = useCallback((item: IBarge) => {
    setSelectedItem(item);
    setViewMode(true);
    setDialogOpen(true);
  }, []);
  const handleEdit = useCallback((item: IBarge) => {
    setSelectedItem(item);
    setViewMode(false);
    setDialogOpen(true);
  }, []);
  const handleDelete = useCallback((item: IBarge) => {
  ...
  const handleDeleteConfirm = useCallback(async () => {
    ...
    try {
      await deleteMutation.mutateAsync(String(itemToDelete.bargeId));
      await invalidateList();
      setDeleteDialogOpen(false);
      setItemToDelete(null);
    } catch {}
  }, [itemToDelete, deleteMutation, invalidateList]);
  const handleFormSubmit = useCallback((payload: Partial<IBarge>) => {
  ...
  return (
    <div className="flex flex-col gap-4 p-4">
            ...
            <div className="space-y-4 py-2">
              <div className="grid grid-cols-2 gap-4 text-sm">
                <div>
                  ...
                  <p className="font-medium">{selectedItem.bargeCode}</p>
                </div>
                <div>
                  <span className="text-slate-500 dark:text-slate-400">
                    Name
      ...
      <ConfirmationDialog
        ...
        type="save"
        title={selectedItem ? "Update Barge" : "Create Barge"}
        message={
          selectedItem
            ? `Update "${selectedItem.bargeName}"?`
            : "Create this barge?"
        }
