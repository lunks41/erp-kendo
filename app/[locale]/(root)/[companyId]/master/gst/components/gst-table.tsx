...
export interface GstTableProps {
  data: IGst[];
  totalRecords?: number;
  onView: (item: IGst) => void;
  onEdit: (item: IGst) => void;
  onDelete: (item: IGst) => void;
  onAdd?: () => void;
  onRefresh?: () => void;
  searchFilter?: string;
...
function GstTableInner(props: GstTableProps) {
  ...
  const defaultColumns: MasterDataGridColumn[] = useMemo(
    () => [
      { field: "gstId", title: "Id", width: 80, hidden: true },
      { field: "gstCode", title: tc("code"), width: 100, minWidth: 80 },
      { field: "gstName", title: tc("name"), flex: true, minWidth: 150 },
      {
        field: "isActive",
        title: tc("active"),
        width: 100,
        ...
        cells: {
          data: (p) => (
            <td {...p.tdProps} className="k-table-td whitespace-nowrap">
              {formatDateTime((p.dataItem as IGst).createDate, datetimeFormat)}
            </td>
          ),
        },
        media: "(min-width: 992px)",
      ...
      },
      {
        ...
        cells: {
          data: (p) => (
            <td {...p.tdProps} className="k-table-td whitespace-nowrap">
              {formatDateTime((p.dataItem as IGst).editDate, datetimeFormat)}
            </td>
          ),
        },
        media: "(min-width: 1200px)",
  ...
  return (
    <MasterDataGrid
      ...
      dataItemKey="gstId"
      actions={{
        onView: (i) => onView(i as unknown as IGst),
        onEdit: (i) => onEdit(i as unknown as IGst),
        onDelete: (i) => onDelete(i as unknown as IGst),
      }}
      pageable
      groupable={false}
      pageSize={pageSize}
...
export const GstTable = memo(GstTableInner);
