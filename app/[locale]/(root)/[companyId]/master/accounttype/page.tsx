...
import type { IAccountType } from "@/interfaces/accounttype";
...
import { AccountTypeForm } from "./components/accounttype-form";
import { AccountTypeTable } from "./components/accounttype-table";
import { useNamespaceTranslations } from "@/hooks/use-form-translations";

export default function AccountTypeMasterPage() {
  const t = useNamespaceTranslations("accounttype");
  ...
  const [itemToDelete, setItemToDelete] = useState<IAccountType | null>(null);
  const [saveDialogOpen, setSaveDialogOpen] = useState(false);
  const [pendingSaveData, setPendingSaveData] =
    useState<Partial<IAccountType> | null>(null);
  const [selectedItem, setSelectedItem] = useState<IAccountType | null>(null);
  ...
  const { data: response, isLoading } = useGetWithPagination<IAccountType>(
    `${AccountType.get}`,
    "accountTypes",
    searchFilter,
    currentPage,
  ...
  const saveMutation = usePersist<IAccountType>(AccountType.add);
  ...
  const handleView = useCallback((item: IAccountType) => {
    setSelectedItem(item);
    setViewMode(true);
    setDialogOpen(true);
  }, []);
  const handleEdit = useCallback((item: IAccountType) => {
    setSelectedItem(item);
    setViewMode(false);
    setDialogOpen(true);
  }, []);
  const handleDelete = useCallback((item: IAccountType) => {
  ...
  const handleDeleteConfirm = useCallback(async () => {
    ...
    try {
      await deleteMutation.mutateAsync(String(itemToDelete.accTypeId));
      await invalidateList();
      setDeleteDialogOpen(false);
      setItemToDelete(null);
    } catch {}
  }, [itemToDelete, deleteMutation, invalidateList]);
  const handleFormSubmit = useCallback((payload: Partial<IAccountType>) => {
    ...
    <div className="flex flex-col gap-4 p-4">
      <div>
        <h1 className="flex items-center gap-1.5 text-lg font-semibold text-slate-900 dark:text-white">
          <Type className="h-5 w-5 text-rose-500" />
          {t("title")}
        </h1>
      ...
      {dialogOpen && (
        ...
        >
          {viewMode && selectedItem ? (
            <div className="space-y-4 py-2">
              <div className="grid grid-cols-2 gap-4 text-sm">
                <div>
                  ...
                  <p className="font-medium">{selectedItem.accTypeCode}</p>
                </div>
                <div>
                  <span className="text-slate-500 dark:text-slate-400">
                    Name
            ...
            <AccountTypeForm
              key={selectedItem?.accTypeId ?? "new"}
      ...
      <ConfirmationDialog
        ...
        type="save"
        title={selectedItem ? "Update Account Type" : "Create Account Type"}
        message={
          selectedItem
            ? `Update "${selectedItem.accTypeName}"?`
            : "Create this account type?"
        }
      ...
      <ConfirmationDialog
        ...
        message={
          itemToDelete
            ? `Delete "${itemToDelete.accTypeName}"?`
            : "Delete this item?"
        }
