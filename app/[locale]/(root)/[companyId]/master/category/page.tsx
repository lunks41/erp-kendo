...
export default function CategoryMasterPage() {
  ...
  const categoryPermission = getPermissions(moduleId, transactionId);
  const hasNoRights =
    mounted && permissionsLoaded && categoryPermission === undefined;

  const [searchFilter, setSearchFilter] = useState("");
  const [searchInput, setSearchInput] = useState("");
  const [dialogOpen, setDialogOpen] = useState(false);
  const [deleteDialogOpen, setDeleteDialogOpen] = useState(false);
  const [itemToDelete, setItemToDelete] = useState<ICategory | null>(null);
  ...
  const { data: response, isLoading } = useGetWithPagination<ICategory>(
    `${Category.get}`,
    "categories",
    searchFilter,
    currentPage,
  ...
  const saveMutation = usePersist<ICategory>(Category.add);
  ...
  const handleView = useCallback((item: ICategory) => {
  ...
  const handleDeleteConfirm = useCallback(async () => {
    ...
    try {
      await deleteMutation.mutateAsync(String(itemToDelete.categoryId));
  ...
  const handleFormSubmit = useCallback((payload: Partial<ICategory>) => {
  ...
  return (
    <div className="flex flex-col gap-4 p-4">
            ...
            <div className="space-y-4 py-2">
              <div className="grid grid-cols-2 gap-4 text-sm">
                <div>
                  ...
                  <p className="font-medium">{selectedItem.categoryCode}</p>
                </div>
                <div>
                  <span className="text-slate-500 dark:text-slate-400">
                    Name
      ...
      <ConfirmationDialog
        ...
        message={
          selectedItem
            ? `Are you sure you want to update category "${selectedItem.categoryName}"?`
      ...
      <ConfirmationDialog
        ...
        message={
          itemToDelete
            ? `Are you sure you want to delete category "${itemToDelete.categoryName}"? This action cannot be undone.`
