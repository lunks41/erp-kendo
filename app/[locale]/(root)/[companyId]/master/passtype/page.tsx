...
import type { IPassType } from "@/interfaces/pass-type";
import { ConfirmationDialog } from "@/components/ui/confirmation";
import { TableSkeleton } from "@/components/skeleton";
import { PassTypeForm } from "./components/passtype-form";
import { MasterTransactionId, ModuleId } from "@/lib/utils";
import { usePermissionStore } from "@/stores/permission-store";
import { useUserSettingDefaults } from "@/hooks/use-settings";
import { PassTypeTable } from "./components/passtype-table";
import { useNamespaceTranslations } from "@/hooks/use-form-translations";

export default function PassTypeMasterPage() {
  const t = useNamespaceTranslations("passType");
  ...
  const [itemToDelete, setItemToDelete] = useState<IPassType | null>(null);
  const [saveDialogOpen, setSaveDialogOpen] = useState(false);
  const [pendingSaveData, setPendingSaveData] =
    useState<Partial<IPassType> | null>(null);
  const [selectedItem, setSelectedItem] = useState<IPassType | null>(null);
  const [viewMode, setViewMode] = useState(false);
  const [currentPage, setCurrentPage] = useState(1);
  const [pageSize, setPageSize] = useState<number | null>(null);
  const effectivePageSize =
    pageSize ?? (defaults?.common?.masterGridTotalRecords || 50);
  const { data: response, isLoading } = useGetWithPagination<IPassType>(
    `${PassType.get}`,
    "passTypes",
    searchFilter,
    currentPage,
  ...
  const saveMutation = usePersist<IPassType>(PassType.add);
  ...
  const handleView = useCallback((item: IPassType) => {
    setSelectedItem(item);
    setViewMode(true);
    setDialogOpen(true);
  }, []);
  const handleEdit = useCallback((item: IPassType) => {
    setSelectedItem(item);
    setViewMode(false);
    setDialogOpen(true);
  }, []);
  const handleDelete = useCallback((item: IPassType) => {
  ...
  const handleDeleteConfirm = useCallback(async () => {
    ...
    try {
      await deleteMutation.mutateAsync(String(itemToDelete.passTypeId));
      await invalidateList();
      setDeleteDialogOpen(false);
      setItemToDelete(null);
    } catch {}
  }, [itemToDelete, deleteMutation, invalidateList]);
  const handleFormSubmit = useCallback((payload: Partial<IPassType>) => {
  ...
  return (
    <div className="flex flex-col gap-4 p-4">
      ...
      {dialogOpen && (
        ...
        >
          {viewMode && selectedItem ? (
            <div className="space-y-4 py-2">
              <div className="grid grid-cols-2 gap-4 text-sm">
                <div>
                  ...
                  <p className="font-medium">{selectedItem.passTypeCode}</p>
                </div>
                <div>
                  <span className="text-slate-500 dark:text-slate-400">
                    Name
                  </span>
                  <p className="font-medium">{selectedItem.passTypeName}</p>
                </div>
                <div>
                  <span className="text-slate-500 dark:text-slate-400">
                    Seq No
            ...
            <PassTypeForm
              key={selectedItem?.passTypeId ?? "new"}
      ...
      <ConfirmationDialog
        ...
        type="save"
        title={selectedItem ? "Update Pass Type" : "Create Pass Type"}
        message={
          selectedItem
            ? `Update "${selectedItem.passTypeName}"?`
            : "Create this pass type?"
        }
      ...
      <ConfirmationDialog
        ...
        message={
          itemToDelete
            ? `Delete "${itemToDelete.passTypeName}"?`
            : "Delete this item?"
        }
