...
export default function CurrencyMasterPage() {
  const t = useNamespaceTranslations("currency");
  const moduleId = ModuleId.master;
  const transactionId = MasterTransactionId.currency;
  ...
  const [itemToDelete, setItemToDelete] = useState<ICurrency | null>(null);
  const [saveDialogOpen, setSaveDialogOpen] = useState(false);
  const [pendingSaveData, setPendingSaveData] =
    useState<Partial<ICurrency> | null>(null);
  const [selectedItem, setSelectedItem] = useState<ICurrency | null>(null);
  ...
  const { data: response, isLoading } = useGetWithPagination<ICurrency>(
    `${Currency.get}`,
    "currencies",
    searchFilter,
    currentPage,
  ...
  const saveMutation = usePersist<ICurrency>(Currency.add);
  ...
  const handleView = useCallback((item: ICurrency) => {
    setSelectedItem(item);
    setViewMode(true);
    setDialogOpen(true);
  }, []);
  const handleEdit = useCallback((item: ICurrency) => {
    setSelectedItem(item);
    setViewMode(false);
    setDialogOpen(true);
  }, []);
  const handleDelete = useCallback((item: ICurrency) => {
  ...
  const handleFormSubmit = useCallback((payload: Partial<ICurrency>) => {
  ...
  return (
    <div className="flex flex-col gap-4 p-4">
      ...
      {dialogOpen && (
        ...
        >
          {viewMode && selectedItem ? (
            <div className="space-y-4 py-2">
              <div className="grid grid-cols-2 gap-4 text-sm">
                <div>
                  ...
                  <p className="font-medium">{selectedItem.currencyCode}</p>
                </div>
                <div>
                  <span className="text-slate-500 dark:text-slate-400">
                    Name
                ...
                <div>
                  ...
                  <p className="font-medium">
                    {selectedItem.isMultiply ? "Yes" : "No"}
                  </p>
                </div>
            ...
            <CurrencyForm
              key={selectedItem?.currencyId ?? "new"}
      ...
      <ConfirmationDialog
        ...
        type="save"
        title={selectedItem ? "Update Currency" : "Create Currency"}
        message={
          selectedItem
            ? `Update "${selectedItem.currencyName}"?`
      ...
      <ConfirmationDialog
        ...
        message={
          itemToDelete
            ? `Delete "${itemToDelete.currencyName}"?`
            : "Delete this item?"
        }
