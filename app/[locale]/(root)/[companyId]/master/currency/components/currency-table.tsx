...
export interface CurrencyTableProps {
  data: ICurrency[];
  totalRecords?: number;
  onView: (item: ICurrency) => void;
  onEdit: (item: ICurrency) => void;
  onDelete: (item: ICurrency) => void;
  onAdd?: () => void;
  onRefresh?: () => void;
  searchFilter?: string;
...
function CurrencyTableInner(props: CurrencyTableProps) {
  ...
  const defaultColumns: MasterDataGridColumn[] = useMemo(
    () => [
      { field: "currencyId", title: "Id", width: 80, hidden: true },
      { field: "currencyCode", title: tc("code"), width: 100, minWidth: 80 },
      { field: "currencyName", title: tc("name"), flex: true, minWidth: 150 },
      {
        field: "isActive",
        title: tc("active"),
        width: 100,
        ...
        cells: {
          data: (props) => {
            const isActive = (props.dataItem as ICurrency).isActive;
        ...
        cells: {
          data: (p) => (
            <td {...p.tdProps} className="k-table-td whitespace-nowrap">
              {formatDateTime(
                (p.dataItem as ICurrency).createDate,
                datetimeFormat,
              )}
            </td>
          ),
      ...
      },
      {
        ...
        cells: {
          data: (p) => (
            <td {...p.tdProps} className="k-table-td whitespace-nowrap">
              {formatDateTime(
                (p.dataItem as ICurrency).editDate,
                datetimeFormat,
              )}
            </td>
          ),
  ...
  return (
    <MasterDataGrid
      ...
      actions={{
        onView: (i) => onView(i as unknown as ICurrency),
        onEdit: (i) => onEdit(i as unknown as ICurrency),
        onDelete: (i) => onDelete(i as unknown as ICurrency),
      }}
      pageable
      groupable={false}
      pageSize={pageSize}
...
export const CurrencyTable = memo(CurrencyTableInner);
