...
export interface CoaCategory3TableProps {
  data: ICoaCategory3[];
  totalRecords?: number;
  onView: (item: ICoaCategory3) => void;
  onEdit: (item: ICoaCategory3) => void;
  onDelete: (item: ICoaCategory3) => void;
  onAdd?: () => void;
  onRefresh?: () => void;
  searchFilter?: string;
...
function CoaCategory3TableInner(props: CoaCategory3TableProps) {
  ...
  const defaultColumns: MasterDataGridColumn[] = useMemo(
    () => [
      ...
      { field: "seqNo", title: tc("seqNo"), width: 90, minWidth: 80 },
      {
        ...
        cells: {
          data: (p) => {
            const isActive = (p.dataItem as ICoaCategory3).isActive;
        ...
        cells: {
          data: (p) => (
            <td {...p.tdProps} className="k-table-td whitespace-nowrap">
              {formatDateTime((p.dataItem as ICoaCategory3).createDate, datetimeFormat)}
            </td>
          ),
        },
        media: "(min-width: 992px)",
      ...
      },
      {
        ...
        cells: {
          data: (p) => (
            <td {...p.tdProps} className="k-table-td whitespace-nowrap">
              {formatDateTime((p.dataItem as ICoaCategory3).editDate, datetimeFormat)}
            </td>
          ),
        },
        media: "(min-width: 1200px)",
  ...
  return (
    <MasterDataGrid
      ...
      actions={{
        onView: (i) => onView(i as unknown as ICoaCategory3),
        onEdit: (i) => onEdit(i as unknown as ICoaCategory3),
        onDelete: (i) => onDelete(i as unknown as ICoaCategory3),
      }}
      pageable
      groupable={false}
      pageSize={pageSize}
...
export const CoaCategory3Table = memo(CoaCategory3TableInner);
