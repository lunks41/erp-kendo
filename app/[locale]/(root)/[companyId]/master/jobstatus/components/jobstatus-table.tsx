...
export interface JobStatusTableProps {
  data: IJobStatus[];
  totalRecords?: number;
  onView: (item: IJobStatus) => void;
  onEdit: (item: IJobStatus) => void;
  onDelete: (item: IJobStatus) => void;
  onAdd?: () => void;
  onRefresh?: () => void;
  searchFilter?: string;
...
function JobStatusTableInner(props: JobStatusTableProps) {
  ...
  const defaultColumns: MasterDataGridColumn[] = useMemo(
    () => [
      { field: "jobStatusId", title: "Id", width: 80, hidden: true },
      { field: "jobStatusCode", title: tc("code"), width: 100, minWidth: 80 },
      { field: "jobStatusName", title: tc("name"), flex: true, minWidth: 150 },
      {
        field: "isActive",
        title: tc("active"),
        width: 100,
        ...
        cells: {
          data: (props) => {
            const isActive = (props.dataItem as IJobStatus).isActive;
        ...
        cells: {
          data: (p) => (
            <td {...p.tdProps} className="k-table-td whitespace-nowrap">
              {formatDateTime(
                (p.dataItem as IJobStatus).createDate,
                datetimeFormat,
              )}
            </td>
          ),
      ...
      },
      {
        ...
        cells: {
          data: (p) => (
            <td {...p.tdProps} className="k-table-td whitespace-nowrap">
              {formatDateTime(
                (p.dataItem as IJobStatus).editDate,
                datetimeFormat,
              )}
            </td>
          ),
  ...
  return (
    <MasterDataGrid
      ...
      dataItemKey="jobStatusId"
      actions={{
        onView: (i) => onView(i as unknown as IJobStatus),
        onEdit: (i) => onEdit(i as unknown as IJobStatus),
        onDelete: (i) => onDelete(i as unknown as IJobStatus),
      }}
      pageable
      groupable={false}
      pageSize={pageSize}
...
export const JobStatusTable = memo(JobStatusTableInner);
