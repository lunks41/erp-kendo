...
export interface PaymentTypeTableProps {
  data: IPaymentType[];
  totalRecords?: number;
  onView: (item: IPaymentType) => void;
  onEdit: (item: IPaymentType) => void;
  onDelete: (item: IPaymentType) => void;
  onAdd?: () => void;
  onRefresh?: () => void;
  searchFilter?: string;
...
function PaymentTypeTableInner(props: PaymentTypeTableProps) {
  ...
  const defaultColumns: MasterDataGridColumn[] = useMemo(
    () => [
      { field: "paymentTypeId", title: "Id", width: 80, hidden: true },
      { field: "paymentTypeCode", title: tc("code"), width: 100, minWidth: 80 },
      {
      ...
      },
      {
        ...
        cells: {
          data: (props) => {
            const isActive = (props.dataItem as IPaymentType).isActive;
        ...
        cells: {
          data: (p) => (
            <td {...p.tdProps} className="k-table-td whitespace-nowrap">
              {formatDateTime(
                (p.dataItem as IPaymentType).createDate,
                datetimeFormat,
              )}
            </td>
          ),
      ...
      },
      {
        ...
        cells: {
          data: (p) => (
            <td {...p.tdProps} className="k-table-td whitespace-nowrap">
              {formatDateTime(
                (p.dataItem as IPaymentType).editDate,
                datetimeFormat,
              )}
            </td>
          ),
  ...
  return (
    <MasterDataGrid
      ...
      actions={{
        onView: (i) => onView(i as unknown as IPaymentType),
        onEdit: (i) => onEdit(i as unknown as IPaymentType),
        onDelete: (i) => onDelete(i as unknown as IPaymentType),
      }}
      pageable
      groupable={false}
      pageSize={pageSize}
...
export const PaymentTypeTable = memo(PaymentTypeTableInner);
