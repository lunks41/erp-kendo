...
import { PaymentTypeForm } from "./components/paymenttype-form";
import { MasterTransactionId, ModuleId } from "@/lib/utils";
import { usePermissionStore } from "@/stores/permission-store";
import { useUserSettingDefaults } from "@/hooks/use-settings";
import { PaymentTypeTable } from "./components/paymenttype-table";
...
export default function PaymentTypeMasterPage() {
  ...
  const [itemToDelete, setItemToDelete] = useState<IPaymentType | null>(null);
  const [saveDialogOpen, setSaveDialogOpen] = useState(false);
  const [pendingSaveData, setPendingSaveData] =
    useState<Partial<IPaymentType> | null>(null);
  const [selectedItem, setSelectedItem] = useState<IPaymentType | null>(null);
  const [viewMode, setViewMode] = useState(false);
  const [currentPage, setCurrentPage] = useState(1);
  const [pageSize, setPageSize] = useState<number | null>(null);
  const effectivePageSize =
    pageSize ?? (defaults?.common?.masterGridTotalRecords || 50);
  const { data: response, isLoading } = useGetWithPagination<IPaymentType>(
    `${PaymentType.get}`,
    "paymentTypes",
    searchFilter,
    currentPage,
  ...
  const saveMutation = usePersist<IPaymentType>(PaymentType.add);
  ...
  const handleView = useCallback((item: IPaymentType) => {
    setSelectedItem(item);
    setViewMode(true);
    setDialogOpen(true);
  }, []);
  const handleEdit = useCallback((item: IPaymentType) => {
    setSelectedItem(item);
    setViewMode(false);
    setDialogOpen(true);
  }, []);
  const handleDelete = useCallback((item: IPaymentType) => {
  ...
  const handleDeleteConfirm = useCallback(async () => {
    ...
    try {
      await deleteMutation.mutateAsync(String(itemToDelete.paymentTypeId));
      await invalidateList();
      setDeleteDialogOpen(false);
      setItemToDelete(null);
    } catch {}
  }, [itemToDelete, deleteMutation, invalidateList]);
  const handleFormSubmit = useCallback((payload: Partial<IPaymentType>) => {
  ...
  return (
    <div className="flex flex-col gap-4 p-4">
      ...
      {dialogOpen && (
        ...
        >
          {viewMode && selectedItem ? (
            <div className="space-y-4 py-2">
              <div className="grid grid-cols-2 gap-4 text-sm">
                <div>
                  ...
                  <p className="font-medium">{selectedItem.paymentTypeCode}</p>
                </div>
                <div>
                  <span className="text-slate-500 dark:text-slate-400">
                    Name
                  </span>
                  <p className="font-medium">{selectedItem.paymentTypeName}</p>
                </div>
            ...
            <PaymentTypeForm
              key={selectedItem?.paymentTypeId ?? "new"}
      ...
      <ConfirmationDialog
        ...
        type="save"
        title={selectedItem ? "Update Payment Type" : "Create Payment Type"}
        message={
          selectedItem
            ? `Update "${selectedItem.paymentTypeName}"?`
            : "Create this payment type?"
        }
      ...
      <ConfirmationDialog
        ...
        message={
          itemToDelete
            ? `Delete "${itemToDelete.paymentTypeName}"?`
            : "Delete this item?"
        }
