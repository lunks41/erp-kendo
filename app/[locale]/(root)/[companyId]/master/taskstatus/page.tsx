...
import { TaskStatusForm } from "./components/taskstatus-form";
import { MasterTransactionId, ModuleId } from "@/lib/utils";
import { usePermissionStore } from "@/stores/permission-store";
import { useUserSettingDefaults } from "@/hooks/use-settings";
import { TaskStatusTable } from "./components/taskstatus-table";
import { useNamespaceTranslations } from "@/hooks/use-form-translations";

export default function TaskStatusMasterPage() {
  const t = useNamespaceTranslations("taskStatus");
  ...
  const [itemToDelete, setItemToDelete] = useState<ITaskStatus | null>(null);
  const [saveDialogOpen, setSaveDialogOpen] = useState(false);
  const [pendingSaveData, setPendingSaveData] =
    useState<Partial<ITaskStatus> | null>(null);
  const [selectedItem, setSelectedItem] = useState<ITaskStatus | null>(null);
  const [viewMode, setViewMode] = useState(false);
  const [currentPage, setCurrentPage] = useState(1);
  const [pageSize, setPageSize] = useState<number | null>(null);
  const effectivePageSize =
    pageSize ?? (defaults?.common?.masterGridTotalRecords || 50);
  const { data: response, isLoading } = useGetWithPagination<ITaskStatus>(
    `${TaskStatus.get}`,
    "taskStatuses",
    searchFilter,
    currentPage,
  ...
  const saveMutation = usePersist<ITaskStatus>(TaskStatus.add);
  ...
  const handleView = useCallback((item: ITaskStatus) => {
    setSelectedItem(item);
    setViewMode(true);
    setDialogOpen(true);
  }, []);
  const handleEdit = useCallback((item: ITaskStatus) => {
    setSelectedItem(item);
    setViewMode(false);
    setDialogOpen(true);
  }, []);
  const handleDelete = useCallback((item: ITaskStatus) => {
  ...
  const handleDeleteConfirm = useCallback(async () => {
    ...
    try {
      await deleteMutation.mutateAsync(String(itemToDelete.taskStatusId));
      await invalidateList();
      setDeleteDialogOpen(false);
      setItemToDelete(null);
    } catch {}
  }, [itemToDelete, deleteMutation, invalidateList]);
  const handleFormSubmit = useCallback((payload: Partial<ITaskStatus>) => {
  ...
  return (
    <div className="flex flex-col gap-4 p-4">
      ...
      {dialogOpen && (
        ...
        >
          {viewMode && selectedItem ? (
            <div className="space-y-4 py-2">
              <div className="grid grid-cols-2 gap-4 text-sm">
                <div>
                  ...
                  <p className="font-medium">{selectedItem.taskStatusCode}</p>
                </div>
                <div>
                  <span className="text-slate-500 dark:text-slate-400">
                    Name
                  </span>
                  <p className="font-medium">{selectedItem.taskStatusName}</p>
                </div>
                <div>
                  <span className="text-slate-500 dark:text-slate-400">
                    Seq No
            ...
            <TaskStatusForm
              key={selectedItem?.taskStatusId ?? "new"}
      ...
      <ConfirmationDialog
        ...
        type="save"
        title={selectedItem ? t("updateTaskStatus") : t("createTaskStatus")}
        message={
          selectedItem
            ? `Update "${selectedItem.taskStatusName}"?`
            : "Create this task status?"
        }
      ...
      <ConfirmationDialog
        ...
        message={
          itemToDelete
            ? `Delete "${itemToDelete.taskStatusName}"?`
            : "Delete this item?"
        }
