...
export interface SubCategoryTableProps {
  data: ISubCategory[];
  totalRecords?: number;
  onView: (item: ISubCategory) => void;
  onEdit: (item: ISubCategory) => void;
  onDelete: (item: ISubCategory) => void;
  onAdd?: () => void;
  onRefresh?: () => void;
  searchFilter?: string;
...
function SubCategoryTableInner(props: SubCategoryTableProps) {
  ...
  const defaultColumns: MasterDataGridColumn[] = useMemo(
    () => [
      { field: "subCategoryId", title: "Id", width: 80, hidden: true },
      { field: "subCategoryCode", title: tc("code"), width: 100, minWidth: 80 },
      {
      ...
      },
      {
        ...
        cells: {
          data: (props) => {
            const isActive = (props.dataItem as ISubCategory).isActive;
        ...
        cells: {
          data: (p) => (
            <td {...p.tdProps} className="k-table-td whitespace-nowrap">
              {formatDateTime(
                (p.dataItem as ISubCategory).createDate,
                datetimeFormat,
              )}
            </td>
          ),
      ...
      },
      {
        ...
        cells: {
          data: (p) => (
            <td {...p.tdProps} className="k-table-td whitespace-nowrap">
              {formatDateTime(
                (p.dataItem as ISubCategory).editDate,
                datetimeFormat,
              )}
            </td>
          ),
  ...
  return (
    <MasterDataGrid
      ...
      actions={{
        onView: (i) => onView(i as unknown as ISubCategory),
        onEdit: (i) => onEdit(i as unknown as ISubCategory),
        onDelete: (i) => onDelete(i as unknown as ISubCategory),
      }}
      pageable
      groupable={false}
      pageSize={pageSize}
...
export const SubCategoryTable = memo(SubCategoryTableInner);
