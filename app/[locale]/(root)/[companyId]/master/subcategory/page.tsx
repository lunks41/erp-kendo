...
export default function SubCategoryMasterPage() {
  ...
  const [pendingSaveData, setPendingSaveData] =
    useState<Partial<ISubCategory> | null>(null);
  ...
  const { data: response, isLoading } = useGetWithPagination<ISubCategory>(
    `${SubCategory.get}`,
    "subCategories",
    searchFilter,
    currentPage,
  ...
  const saveMutation = usePersist<ISubCategory>(SubCategory.add);
  ...
  const handleView = useCallback((item: ISubCategory) => {
    setSelectedItem(item);
    setViewMode(true);
    setDialogOpen(true);
  }, []);
  const handleEdit = useCallback((item: ISubCategory) => {
    setSelectedItem(item);
    setViewMode(false);
    setDialogOpen(true);
  }, []);
  const handleDelete = useCallback((item: ISubCategory) => {
  ...
  const handleDeleteConfirm = useCallback(async () => {
    ...
    try {
      await deleteMutation.mutateAsync(String(itemToDelete.subCategoryId));
      await invalidateList();
      setDeleteDialogOpen(false);
      setItemToDelete(null);
    } catch {}
  }, [itemToDelete, deleteMutation, invalidateList]);
  const handleFormSubmit = useCallback((payload: Partial<ISubCategory>) => {
  ...
  return (
    <div className="flex flex-col gap-4 p-4">
      ...
      {dialogOpen && (
        ...
        >
          {viewMode && selectedItem ? (
            <div className="space-y-4 py-2">
              <div className="grid grid-cols-2 gap-4 text-sm">
                <div>
                  ...
                  <p className="font-medium">{selectedItem.subCategoryCode}</p>
                </div>
                <div>
                  <span className="text-slate-500 dark:text-slate-400">
                    {tc("name")}
                  </span>
                  <p className="font-medium">{selectedItem.subCategoryName}</p>
                </div>
            ...
            <SubCategoryForm
              key={selectedItem?.subCategoryId ?? "new"}
      ...
      <ConfirmationDialog
        ...
        type="save"
        title={selectedItem ? t("updateSubCategory") : t("createSubCategory")}
        message={
          selectedItem
            ? `Update "${selectedItem.subCategoryName}"?`
            : "Create this sub category?"
        }
      ...
      <ConfirmationDialog
        ...
        message={
          itemToDelete
            ? `Delete "${itemToDelete.subCategoryName}"?`
            : "Delete this item?"
        }
