...
export interface UomTableProps {
  data: IUom[];
  totalRecords?: number;
  onView: (item: IUom) => void;
  onEdit: (item: IUom) => void;
  onDelete: (item: IUom) => void;
  onAdd?: () => void;
  onRefresh?: () => void;
  searchFilter?: string;
...
function UomTableInner(props: UomTableProps) {
  ...
  const defaultColumns: MasterDataGridColumn[] = useMemo(
    () => [
      { field: "uomId", title: "Id", width: 80, hidden: true },
      { field: "uomCode", title: tc("code"), width: 100, minWidth: 80 },
      { field: "uomName", title: tc("name"), flex: true, minWidth: 150 },
      {
        field: "isActive",
        title: tc("active"),
        width: 100,
        ...
        cells: {
          data: (props) => {
            const isActive = (props.dataItem as IUom).isActive;
        ...
        cells: {
          data: (p) => (
            <td {...p.tdProps} className="k-table-td whitespace-nowrap">
              {formatDateTime((p.dataItem as IUom).createDate, datetimeFormat)}
            </td>
          ),
        },
        media: "(min-width: 992px)",
      ...
      },
      {
        ...
        cells: {
          data: (p) => (
            <td {...p.tdProps} className="k-table-td whitespace-nowrap">
              {formatDateTime((p.dataItem as IUom).editDate, datetimeFormat)}
            </td>
          ),
        },
        media: "(min-width: 1200px)",
  ...
  return (
    <MasterDataGrid
      ...
      dataItemKey="uomId"
      actions={{
        onView: (i) => onView(i as unknown as IUom),
        onEdit: (i) => onEdit(i as unknown as IUom),
        onDelete: (i) => onDelete(i as unknown as IUom),
      }}
      pageable
      groupable={false}
      pageSize={pageSize}
...
export const UomTable = memo(UomTableInner);
