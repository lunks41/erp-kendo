...
export interface ConsignmentTypeTableProps {
  data: IConsignmentType[];
  totalRecords?: number;
  onView: (item: IConsignmentType) => void;
  onEdit: (item: IConsignmentType) => void;
  onDelete: (item: IConsignmentType) => void;
  onAdd?: () => void;
  onRefresh?: () => void;
  searchFilter?: string;
...
function ConsignmentTypeTableInner(props: ConsignmentTypeTableProps) {
  ...
  const defaultColumns: MasterDataGridColumn[] = useMemo(
    () => [
      { field: "consignmentTypeId", title: "Id", width: 80, hidden: true },
      {
        ...
        cells: {
          data: (p) => (
            <td {...p.tdProps} className="k-table-td whitespace-nowrap">
              {formatDateTime(
                (p.dataItem as IConsignmentType).createDate,
                datetimeFormat,
              )}
            </td>
          ),
      ...
      },
      {
        ...
        cells: {
          data: (p) => (
            <td {...p.tdProps} className="k-table-td whitespace-nowrap">
              {formatDateTime(
                (p.dataItem as IConsignmentType).editDate,
                datetimeFormat,
              )}
            </td>
          ),
  ...
  return (
    <MasterDataGrid
      ...
      dataItemKey="consignmentTypeId"
      actions={{
        onView: (i) => onView(i as unknown as IConsignmentType),
        onEdit: (i) => onEdit(i as unknown as IConsignmentType),
        onDelete: (i) => onDelete(i as unknown as IConsignmentType),
      }}
      pageable
      groupable={false}
      pageSize={pageSize}
...
export const ConsignmentTypeTable = memo(ConsignmentTypeTableInner);
