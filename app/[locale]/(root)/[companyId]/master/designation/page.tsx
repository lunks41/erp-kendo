...
import type { IDesignation } from "@/interfaces/designation";
...
export default function DesignationMasterPage() {
  const t = useNamespaceTranslations("designation");
  ...
  const designationPermission = getPermissions(moduleId, transactionId);
  const hasNoRights =
    mounted && permissionsLoaded && designationPermission === undefined;

  const [searchFilter, setSearchFilter] = useState("");
  const [searchInput, setSearchInput] = useState("");
  const [dialogOpen, setDialogOpen] = useState(false);
  const [deleteDialogOpen, setDeleteDialogOpen] = useState(false);
  const [itemToDelete, setItemToDelete] = useState<IDesignation | null>(null);
  const [saveDialogOpen, setSaveDialogOpen] = useState(false);
  const [pendingSaveData, setPendingSaveData] =
    useState<Partial<IDesignation> | null>(null);
  const [selectedItem, setSelectedItem] = useState<IDesignation | null>(null);
  ...
  const { data: response, isLoading } = useGetWithPagination<IDesignation>(
    `${Designation.get}`,
    "designations",
    searchFilter,
    currentPage,
  ...
  const saveMutation = usePersist<IDesignation>(Designation.add);
  ...
  const handleView = useCallback((item: IDesignation) => {
    setSelectedItem(item);
    setViewMode(true);
    setDialogOpen(true);
  }, []);

  const handleEdit = useCallback((item: IDesignation) => {
    setSelectedItem(item);
    setViewMode(false);
    setDialogOpen(true);
  }, []);

  const handleDelete = useCallback((item: IDesignation) => {
  ...
  const handleDeleteConfirm = useCallback(async () => {
    ...
    try {
      await deleteMutation.mutateAsync(String(itemToDelete.designationId));
  ...
  const handleFormSubmit = useCallback((payload: Partial<IDesignation>) => {
  ...
  return (
    <div className="flex flex-col gap-4 p-4">
            ...
            <div className="space-y-4 py-2">
              <div className="grid grid-cols-2 gap-4 text-sm">
                <div>
                  ...
                  <p className="font-medium">{selectedItem.designationCode}</p>
                </div>
                <div>
                  <span className="text-slate-500 dark:text-slate-400">
                    Name
      ...
      <ConfirmationDialog
        ...
        type="save"
        title={selectedItem ? "Update Designation" : "Create Designation"}
        message={
          selectedItem
            ? `Are you sure you want to update designation "${selectedItem.designationName}"?`
      ...
      <ConfirmationDialog
        ...
        message={
          itemToDelete
            ? `Are you sure you want to delete designation "${itemToDelete.designationName}"? This action cannot be undone.`
