...
import type { ITaxCategory } from "@/interfaces/tax";
import { TaxCategory } from "@/lib/api-routes";
...
export default function TaxCategoryMasterPage() {
  const t = useNamespaceTranslations("taxCategory");
  const tc = useNamespaceTranslations("common");
  const moduleId = ModuleId.master;
  const transactionId = MasterTransactionId.taxCategory;
  ...
  const [itemToDelete, setItemToDelete] = useState<ITaxCategory | null>(null);
  const [saveDialogOpen, setSaveDialogOpen] = useState(false);
  const [pendingSaveData, setPendingSaveData] =
    useState<Partial<ITaxCategory> | null>(null);
  const [selectedItem, setSelectedItem] = useState<ITaxCategory | null>(null);
  const [viewMode, setViewMode] = useState(false);
  const [currentPage, setCurrentPage] = useState(1);
  const [pageSize, setPageSize] = useState<number | null>(null);
  const effectivePageSize =
    pageSize ?? (defaults?.common?.masterGridTotalRecords || 50);
  const { data: response, isLoading } = useGetWithPagination<ITaxCategory>(
    `${TaxCategory.get}`,
    "taxCategory",
    searchFilter,
    currentPage,
  ...
  const saveMutation = usePersist<ITaxCategory>(TaxCategory.add);
  const deleteMutation = useDelete(TaxCategory.delete);
  ...
  const handleView = useCallback((item: ITaxCategory) => {
    setSelectedItem(item);
    setViewMode(true);
    setDialogOpen(true);
  }, []);
  const handleEdit = useCallback((item: ITaxCategory) => {
    setSelectedItem(item);
    setViewMode(false);
    setDialogOpen(true);
  }, []);
  const handleDelete = useCallback((item: ITaxCategory) => {
  ...
  const handleDeleteConfirm = useCallback(async () => {
    ...
    try {
      await deleteMutation.mutateAsync(String(itemToDelete.taxCategoryId));
      await invalidateList();
      setDeleteDialogOpen(false);
      setItemToDelete(null);
    } catch {}
  }, [itemToDelete, deleteMutation, invalidateList]);
  const handleFormSubmit = useCallback((payload: Partial<ITaxCategory>) => {
  ...
  return (
    <div className="flex flex-col gap-4 p-4">
      ...
      {dialogOpen && (
        ...
        >
          {viewMode && selectedItem ? (
            <div className="space-y-4 py-2">
              <div className="grid grid-cols-2 gap-4 text-sm">
                <div>
                  ...
                  <p className="font-medium">{selectedItem.taxCategoryCode}</p>
                </div>
                <div>
                  <span className="text-slate-500 dark:text-slate-400">
                    {t("taxCategoryName")}
                  </span>
                  <p className="font-medium">{selectedItem.taxCategoryName}</p>
                </div>
            ...
            <TaxCategoryForm
              key={selectedItem?.taxCategoryId ?? "new"}
      ...
      <ConfirmationDialog
        ...
        type="save"
        title={selectedItem ? t("updateTaxCategory") : t("createTaxCategory")}
        message={
          selectedItem
            ? `Update "${selectedItem.taxCategoryName}"?`
            : "Create this tax category?"
        }
      ...
      <ConfirmationDialog
        ...
        type="delete"
        title={t("deleteTaxCategory")}
        message={
          itemToDelete
            ? `Delete "${itemToDelete.taxCategoryName}"?`
            : "Delete this item?"
        }
