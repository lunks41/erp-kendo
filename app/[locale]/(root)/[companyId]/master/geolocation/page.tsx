...
import { GeoLocationTable } from "./components/geolocation-table";
import { useNamespaceTranslations } from "@/hooks/use-form-translations";

export default function GeoLocationMasterPage() {
  ...
  const [itemToDelete, setItemToDelete] = useState<IGeoLocation | null>(null);
  const [saveDialogOpen, setSaveDialogOpen] = useState(false);
  const [pendingSaveData, setPendingSaveData] =
    useState<Partial<IGeoLocation> | null>(null);
  const [selectedItem, setSelectedItem] = useState<IGeoLocation | null>(null);
  ...
  const { data: response, isLoading } = useGetWithPagination<IGeoLocation>(
    `${GeoLocation.get}`,
    "geoLocations",
    searchFilter,
    currentPage,
  ...
  const saveMutation = usePersist<IGeoLocation>(GeoLocation.add);
  ...
  const handleView = useCallback((item: IGeoLocation) => {
    setSelectedItem(item);
    setViewMode(true);
    setDialogOpen(true);
  }, []);
  const handleEdit = useCallback((item: IGeoLocation) => {
    setSelectedItem(item);
    setViewMode(false);
    setDialogOpen(true);
  }, []);
  const handleDelete = useCallback((item: IGeoLocation) => {
  ...
  const handleDeleteConfirm = useCallback(async () => {
    ...
    try {
      await deleteMutation.mutateAsync(String(itemToDelete.geoLocationId));
      await invalidateList();
      setDeleteDialogOpen(false);
      setItemToDelete(null);
    } catch {}
  }, [itemToDelete, deleteMutation, invalidateList]);
  const handleFormSubmit = useCallback((payload: Partial<IGeoLocation>) => {
  ...
  return (
    <div className="flex flex-col gap-4 p-4">
      ...
      {dialogOpen && (
        ...
        >
          {viewMode && selectedItem ? (
            <div className="space-y-4 py-2">
              <div className="grid grid-cols-2 gap-4 text-sm">
                <div>
                  ...
                  <p className="font-medium">{selectedItem.geoLocationCode}</p>
                </div>
                <div>
                  <span className="text-slate-500 dark:text-slate-400">
                    Name
                  </span>
                  <p className="font-medium">{selectedItem.geoLocationName}</p>
                </div>
                <div>
                  <span className="text-slate-500 dark:text-slate-400">
                    Port ID
                  </span>
                  <p className="font-medium">{selectedItem.portId}</p>
                </div>
            ...
            <GeoLocationForm
              key={selectedItem?.geoLocationId ?? "new"}
      ...
      <ConfirmationDialog
        ...
        type="save"
        title={selectedItem ? "Update Geo Location" : "Create Geo Location"}
        message={
          selectedItem
            ? `Update "${selectedItem.geoLocationName}"?`
            : "Create this geo location?"
        }
      ...
      <ConfirmationDialog
        ...
        message={
          itemToDelete
            ? `Delete "${itemToDelete.geoLocationName}"?`
            : "Delete this item?"
        }
