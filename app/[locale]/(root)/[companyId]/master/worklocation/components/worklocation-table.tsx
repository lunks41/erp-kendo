...
export interface WorkLocationTableProps {
  data: IWorkLocation[];
  totalRecords?: number;
...
function WorkLocationTableInner(props: WorkLocationTableProps) {
  ...
  const defaultColumns: MasterDataGridColumn[] = useMemo(
    () => [
      { field: "workLocationId", title: "Id", width: 80, hidden: true },
      {
        field: "workLocationCode",
        title: tc("code"),
        width: 100,
        minWidth: 80,
      },
      ...
      },
      {
        ...
        cells: {
          data: (p) => {
            const isActive = (p.dataItem as IWorkLocation).isActive;
        ...
        cells: {
          data: (p) => (
            <td {...p.tdProps} className="k-table-td whitespace-nowrap">
              {formatDateTime((p.dataItem as IWorkLocation).createDate, datetimeFormat)}
            </td>
          ),
        },
        media: "(min-width: 1200px)",
  ...
  return (
    <MasterDataGrid
      ...
      dataItemKey="workLocationId"
      actions={{
        onView: (i) => onView(i as unknown as IWorkLocation),
        onEdit: (i) => onEdit(i as unknown as IWorkLocation),
        onDelete: (i) => onDelete(i as unknown as IWorkLocation),
      }}
      pageable
      groupable={false}
      pageSize={pageSize}
...
export const WorkLocationTable = memo(WorkLocationTableInner);
