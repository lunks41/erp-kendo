...
import type { ITransportLocation } from "@/interfaces/transport-location";
import { ConfirmationDialog } from "@/components/ui/confirmation";
import { TableSkeleton } from "@/components/skeleton";
import { TransportLocationForm } from "./components/transportlocation-form";
import { MasterTransactionId, ModuleId } from "@/lib/utils";
import { usePermissionStore } from "@/stores/permission-store";
import { useUserSettingDefaults } from "@/hooks/use-settings";
import { TransportLocationTable } from "./components/transportlocation-table";
import { useNamespaceTranslations } from "@/hooks/use-form-translations";

export default function TransportLocationMasterPage() {
  const t = useNamespaceTranslations("transportLocation");
  ...
  const [itemToDelete, setItemToDelete] = useState<ITransportLocation | null>(
    null,
  );
  const [saveDialogOpen, setSaveDialogOpen] = useState(false);
  const [pendingSaveData, setPendingSaveData] =
    useState<Partial<ITransportLocation> | null>(null);
  const [selectedItem, setSelectedItem] = useState<ITransportLocation | null>(
    null,
  );
  ...
  const saveMutation = usePersist<ITransportLocation>(TransportLocation.add);
  ...
  const handleView = useCallback((item: ITransportLocation) => {
    setSelectedItem(item);
    setViewMode(true);
    setDialogOpen(true);
  }, []);
  const handleEdit = useCallback((item: ITransportLocation) => {
    setSelectedItem(item);
    setViewMode(false);
    setDialogOpen(true);
  }, []);
  const handleDelete = useCallback((item: ITransportLocation) => {
  ...
  const handleDeleteConfirm = useCallback(async () => {
    ...
    try {
      await deleteMutation.mutateAsync(
        String(itemToDelete.transportLocationId),
      );
  ...
  const handleFormSubmit = useCallback(
    (payload: Partial<ITransportLocation>) => {
  ...
  return (
    <div className="flex flex-col gap-4 p-4">
        ...
        >
          {viewMode && selectedItem ? (
              ...
              <div className="grid grid-cols-2 gap-4 text-sm">
                <div>
                  ...
                  <p className="font-medium">
                    {selectedItem.transportLocationCode}
                  </p>
                </div>
                <div>
                  <span className="text-slate-500 dark:text-slate-400">
                  ...
                  <p className="font-medium">
                    {selectedItem.transportLocationName}
                  </p>
                </div>
                <div>
                  <span className="text-slate-500 dark:text-slate-400">
            ...
            <TransportLocationForm
              key={selectedItem?.transportLocationId ?? "new"}
      ...
      <ConfirmationDialog
        ...
        title={
          selectedItem
            ? t("updateTransportLocation")
            : t("createTransportLocation")
        }
        message={
          selectedItem
            ? `Update "${selectedItem.transportLocationName}"?`
            : "Create this transport location?"
        }
      ...
      <ConfirmationDialog
        ...
        message={
          itemToDelete
            ? `Delete "${itemToDelete.transportLocationName}"?`
            : "Delete this item?"
        }
