...
import type { ICoaCategory2 } from "@/interfaces/coacategory";
import { CoaCategory2 } from "@/lib/api-routes";
...
export default function CoaCategory2MasterPage() {
  const t = useNamespaceTranslations("coaCategory2");
  const tc = useNamespaceTranslations("common");
  const moduleId = ModuleId.master;
  const transactionId = MasterTransactionId.coaCategory2;
  ...
  const [itemToDelete, setItemToDelete] = useState<ICoaCategory2 | null>(null);
  const [saveDialogOpen, setSaveDialogOpen] = useState(false);
  const [pendingSaveData, setPendingSaveData] =
    useState<Partial<ICoaCategory2> | null>(null);
  const [selectedItem, setSelectedItem] = useState<ICoaCategory2 | null>(null);
  const [viewMode, setViewMode] = useState(false);
  const [currentPage, setCurrentPage] = useState(1);
  const [pageSize, setPageSize] = useState<number | null>(null);
  const effectivePageSize =
    pageSize ?? (defaults?.common?.masterGridTotalRecords || 50);
  const { data: response, isLoading } = useGetWithPagination<ICoaCategory2>(
    `${CoaCategory2.get}`,
    "coaCategory2",
    searchFilter,
    currentPage,
  ...
  const saveMutation = usePersist<ICoaCategory2>(CoaCategory2.add);
  const deleteMutation = useDelete(CoaCategory2.delete);
  ...
  const handleView = useCallback((item: ICoaCategory2) => {
    setSelectedItem(item);
    setViewMode(true);
    setDialogOpen(true);
  }, []);
  const handleEdit = useCallback((item: ICoaCategory2) => {
    setSelectedItem(item);
    setViewMode(false);
    setDialogOpen(true);
  }, []);
  const handleDelete = useCallback((item: ICoaCategory2) => {
  ...
  const handleFormSubmit = useCallback((payload: Partial<ICoaCategory2>) => {
  ...
  return (
    <div className="flex flex-col gap-4 p-4">
      ...
      <ConfirmationDialog
        ...
        type="save"
        title={selectedItem ? t("updateCoaCategory2") : t("createCoaCategory2")}
        message={
          selectedItem
            ? `Update "${selectedItem.coaCategoryName}"?`
            : "Create this COA category 2?"
        }
