...
export interface CoaCategory2TableProps {
  data: ICoaCategory2[];
  totalRecords?: number;
  onView: (item: ICoaCategory2) => void;
  onEdit: (item: ICoaCategory2) => void;
  onDelete: (item: ICoaCategory2) => void;
  onAdd?: () => void;
  onRefresh?: () => void;
  searchFilter?: string;
...
function CoaCategory2TableInner(props: CoaCategory2TableProps) {
  ...
  const defaultColumns: MasterDataGridColumn[] = useMemo(
    () => [
      ...
      { field: "seqNo", title: tc("seqNo"), width: 90, minWidth: 80 },
      {
        ...
        cells: {
          data: (p) => {
            const isActive = (p.dataItem as ICoaCategory2).isActive;
        ...
        cells: {
          data: (p) => (
            <td {...p.tdProps} className="k-table-td whitespace-nowrap">
              {formatDateTime((p.dataItem as ICoaCategory2).createDate, datetimeFormat)}
            </td>
          ),
        },
        media: "(min-width: 992px)",
      ...
      },
      {
        ...
        cells: {
          data: (p) => (
            <td {...p.tdProps} className="k-table-td whitespace-nowrap">
              {formatDateTime((p.dataItem as ICoaCategory2).editDate, datetimeFormat)}
            </td>
          ),
        },
        media: "(min-width: 1200px)",
  ...
  return (
    <MasterDataGrid
      ...
      actions={{
        onView: (i) => onView(i as unknown as ICoaCategory2),
        onEdit: (i) => onEdit(i as unknown as ICoaCategory2),
        onDelete: (i) => onDelete(i as unknown as ICoaCategory2),
      }}
      pageable
      groupable={false}
      pageSize={pageSize}
...
export const CoaCategory2Table = memo(CoaCategory2TableInner);
