...
import type { IVATServiceCategory } from "@/interfaces/vat-service-category";
import { ConfirmationDialog } from "@/components/ui/confirmation";
import { TableSkeleton } from "@/components/skeleton";
import { VATServiceCategoryForm } from "./components/vatservicecategory-form";
import { MasterTransactionId, ModuleId } from "@/lib/utils";
import { usePermissionStore } from "@/stores/permission-store";
import { useUserSettingDefaults } from "@/hooks/use-settings";
import { VATServiceCategoryTable } from "./components/vatservicecategory-table";
import { useNamespaceTranslations } from "@/hooks/use-form-translations";

export default function VATServiceCategoryMasterPage() {
  const t = useNamespaceTranslations("vatServiceCategory");
  ...
  const [itemToDelete, setItemToDelete] = useState<IVATServiceCategory | null>(
    null,
  );
  const [saveDialogOpen, setSaveDialogOpen] = useState(false);
  const [pendingSaveData, setPendingSaveData] =
    useState<Partial<IVATServiceCategory> | null>(null);
  const [selectedItem, setSelectedItem] = useState<IVATServiceCategory | null>(
    null,
  );
  ...
  const saveMutation = usePersist<IVATServiceCategory>(VATServiceCategory.add);
  ...
  const handleView = useCallback((item: IVATServiceCategory) => {
    setSelectedItem(item);
    setViewMode(true);
    setDialogOpen(true);
  }, []);
  const handleEdit = useCallback((item: IVATServiceCategory) => {
    setSelectedItem(item);
    setViewMode(false);
    setDialogOpen(true);
  }, []);
  const handleDelete = useCallback((item: IVATServiceCategory) => {
  ...
  const handleDeleteConfirm = useCallback(async () => {
    ...
    try {
      await deleteMutation.mutateAsync(String(itemToDelete.serviceCategoryId));
  ...
  const handleFormSubmit = useCallback(
    (payload: Partial<IVATServiceCategory>) => {
  ...
  return (
    <div className="flex flex-col gap-4 p-4">
      ...
      {dialogOpen && (
        <Dialog
          title={
            selectedItem
              ? viewMode
                ? t("viewVATServiceCategory")
                : t("editVATServiceCategory")
        ...
        >
          {viewMode && selectedItem ? (
              ...
              <div className="grid grid-cols-2 gap-4 text-sm">
                <div>
                  ...
                  <p className="font-medium">
                    {selectedItem.serviceCategoryCode}
                  </p>
                </div>
                <div>
                  <span className="text-slate-500 dark:text-slate-400">
                  ...
                  <p className="font-medium">
                    {selectedItem.serviceCategoryName}
                  </p>
                </div>
                <div>
                  <span className="text-slate-500 dark:text-slate-400">
            ...
            <VATServiceCategoryForm
              key={selectedItem?.serviceCategoryId ?? "new"}
      ...
      <ConfirmationDialog
        ...
        message={
          selectedItem
            ? `Update "${selectedItem.serviceCategoryName}"?`
            : "Create this VAT service category?"
        }
      ...
      <ConfirmationDialog
        ...
        type="delete"
        title={t("deleteVATServiceCategory")}
        message={
          itemToDelete
            ? `Delete "${itemToDelete.serviceCategoryName}"?`
            : "Delete this item?"
        }
