      ...
      (set, get) => ({
        ...
        logIn: async (userName: string, userPassword: string) => {
          ...
          try {
            ...
            if (data.result === 1) {
              debug.log("🔐 LOGIN SUCCESS: User authenticated")
              get().logInSuccess(data.user, data.token, data.refreshToken)

              if (!get().isAppLocked) {
                debug.log("🏢 STEP 1: Fetching companies...")
                await get().getCompanies()
                debug.log("✅ STEP 1 COMPLETE: Companies fetched")
              }
            } else {
              debug.log("🔐 LOGIN FAILED: User not authenticated")
        ...
        getCompanies: async () => {
          ...
          try {
            ...
            if (!get().currentCompany && companies.length > 0) {
              ...
              debug.log("📊 Will fetch decimals: true")
              await get().switchCompany(companies[0].companyId, true)
              debug.log("✅ AUTO-SELECTION COMPLETE")
            }
          } catch {}
        },
        ...
        switchCompany: async (companyId: string, fetchDecimals = true) => {
          debug.log("🏢 SWITCH COMPANY: Starting for company:", companyId)
          debug.log("📊 Switch Parameters:", {
          ...
          if (!currentCompany && companies.length > 0) {
            ...
            // )
            // debug.log("🔗 Expected URL: /company-select")
          }

          try {
            // debug.log("🔍 STEP 2: Validating company ID")
            if (!companyId) {
              debug.error("❌ STEP 2 FAILED: Company ID is required")
              throw new Error("Company ID is required")
            }
            // debug.log("✅ STEP 2 PASSED: Company ID is valid")

            // debug.log("🔄 STEP 3: Checking if company is already selected")
            ...
            // debug.log("✅ STEP 3: Company switch needed")

            // debug.log("🔍 STEP 4: Finding company in available companies")
            const company = companies.find(
              (c) => c.companyId.toString() === companyId
            )
            // debug.log("📊 Company search result:", {
            //   found: !!company,
            ...
            if (!company) {
              debug.error("❌ STEP 4 FAILED: Company not found")
              throw new Error(
                `Company with ID ${companyId} not found. Please select a valid company.`
              )
            }
            // debug.log("✅ STEP 4 PASSED: Company found")

            // debug.log("🔄 STEP 5: Updating sessionStorage (tab-specific)")
            // debug.log("📊 Before sessionStorage update:", {
            //   oldSessionStorage: sessionStorage.getItem("tab_company_id"),
            //   newCompanyId: companyId,
            // })
            get().setCurrentTabCompanyId(companyId)
            // debug.log("📊 After sessionStorage update:", {
            //   newSessionStorage: sessionStorage.getItem("tab_company_id"),
            //   matches: sessionStorage.getItem("tab_company_id") === companyId,
            // })
            // debug.log("✅ STEP 5: sessionStorage updated")

            // debug.log("🔄 STEP 6: Updating Zustand store (global state)")
            // debug.log("📊 Before Zustand update:", {
            //   oldCurrentCompany: currentCompany?.companyId,
            //   newCompanyId: companyId,
            // })
            set({ currentCompany: company })
            // debug.log("📊 After Zustand update:", {
            ...
            if (fetchDecimals) {
              debug.log("📊 Adding getDecimals() to API queue")
            ...
            debug.log("🔐 Adding getUserTransactions() to API queue")
            apiPromises.push(
              get()
                .getUserTransactions()
                .then(() => {})
            ...
            debug.log("🚀 EXECUTING API CALLS IN PARALLEL")
            debug.log("📊 Total API calls:", apiPromises.length)
            ...
            results.forEach((result, index) => {
              ...
              if (result.status === "rejected") {
                debug.error(`❌ ${apiName} failed:`, result.reason)
              } else {
                debug.log(`✅ ${apiName} succeeded`)
              }
            })

            debug.log("✅ COMPANY SWITCH COMPLETED SUCCESSFULLY")
            ...
            debug.error("❌ STEP FAILED: Company switch failed:", error)
        ...
        getDecimals: async () => {
          ...
          if (!currentCompany || !user) {
            debug.warn("⚠️ getDecimals aborted: No company or user")
            return
          }
          ...
          try {
            ...
            debug.log("🔢 getDecimals response:", data)
            debug.log(
              "🔍 Response type:",
            ...
            if (decimaldata.length > 0) {
              debug.log("✅ Decimals loaded successfully:", decimaldata[0])
              get().setDecimals(decimaldata)
            } else {
              debug.warn("⚠️ No decimal data received, using defaults")
              // Use fallback defaults if no data received
              get().setDecimals([getDefaultDecimalSettings()])
            }
          } catch (error) {
            debug.error("❌ Error fetching decimal settings:", error)
        ...
        refreshTokenAutomatically: async () => {
          ...
          try {
            ...
            debug.error("❌ [AuthStore] Token refresh failed:", error)
